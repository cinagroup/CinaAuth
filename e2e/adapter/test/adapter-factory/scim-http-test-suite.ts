...
import { runSCIMLifecycle } from "@cinaauth/test-utils/scim";
import { toNodeHandler } from "cinaauth/node";
import { createHttpTestServer } from "cinaauth/test";
...
export function scimHttpTestSuite(options: SCIMHttpTestSuiteOptions) {
	...
	const scimCinaAuthOptions: CinaAuthOptions = {
		user: {
			additionalFields: {
	...
	return createTestSuite(
		"scim-http",
		{},
		({ adapter, getAuth, modifyCinaAuthOptions }) => ({
			"provisions users and groups through a real HTTP listener": {
				migrateCinaAuth: scimCinaAuthOptions,
				async test() {
					...
					try {
						await modifyCinaAuthOptions(
							{ ...scimCinaAuthOptions, baseURL: listener.url },
							false,
						);
