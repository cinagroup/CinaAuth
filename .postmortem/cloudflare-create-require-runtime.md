...
`cinaauth@1.7.0-beta.4` and `1.7.0-beta.5` reintroduced the Cloudflare
Workers startup crash previously seen in the 1.4.6 release line. The published
`cinaauth` package emitted this shared rolldown runtime helper:
...
crash affected unrelated subpaths such as `cinaauth/db` because the helper
...
3. **PR #9657**: Added `getHttpTestInstance` to `cinaauth/test`. The helper
   statically imported `listhen`, which pulled in CommonJS-oriented Node server
   dependencies when the `cinaauth` multi-entry package was built.
...
`packages/cinaauth/src/test-utils/index.ts` statically re-exported
`http-test-instance.ts`, and that file statically imported `listhen`.

Because `cinaauth/test` is built as an entry in the same `cinaauth`
...
`cinaauth/db` did not import `cinaauth/test`, but still evaluated the
...
but it did not inspect the built `packages/cinaauth/dist/_virtual/_rolldown/runtime.mjs`
...
2. The built `cinaauth` rolldown runtime helper must not contain
...
3. **Do not add external listener/server dependencies to `cinaauth/test`
...
1. Keep the Cloudflare smoke test's direct assertion on
   `packages/cinaauth/dist/_virtual/_rolldown/runtime.mjs`.
2. When reviewing changes to `packages/cinaauth/src/test-utils`, rebuild
   `cinaauth` and scan the generated runtime helper for `createRequire`,
