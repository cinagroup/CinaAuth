...
import type { MemoryDB } from "cinaauth/adapters/memory";
...
function createAuth(database: CinaAuthOptions["database"]) {
	return CinaAuth({
		baseURL: BASE_URL,
		database,
		plugins: [
...
function createSourceUpdateRaceAdapter(data: UserDeletionData) {
	...
	return {
		database: (options: CinaAuthOptions): DBAdapter => {
...
describe("SCIM User concurrency", () => {
	it("retries concurrent first-time links to the same CinaAuth User", async () => {
