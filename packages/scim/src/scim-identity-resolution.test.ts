...
function createIdentityFixture(
	options: {
		...
		databaseHooks?: CinaAuthOptions["databaseHooks"];
...
describe("SCIM explicit identity resolution", () => {
	...
	it("defaults to creating a new CinaAuth User when no resolver is configured", async () => {
	...
	it("revokes email verification whenever a managing source changes the backing email", async () => {
		...
		if (!linkedUser) throw new Error("Expected the linked CinaAuth User");
	...
	it("reprovisions a deleted externalId onto the same CinaAuth User", async () => {
		const { auth, data } = createIdentityFixture();
