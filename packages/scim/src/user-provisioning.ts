import type { AuthContext, DBAdapter, User, Where } from "cinaauth";
...
async function assertConnectionUserAvailable(
	...
	if (existing) {
		throw createSCIMError("CONFLICT", {
			detail: "This connection already provisions the resolved CinaAuth User",
			scimType: "uniqueness",
		});
	}
}
...
async function assertCinaAuthEmailAvailable(
	adapter: Pick<DBAdapter, "findOne">,
	email: string,
	...
	if (existingUser && existingUser.id !== excludeUserId) {
		throw createSCIMError("CONFLICT", {
			detail: "A CinaAuth User already uses this email",
			scimType: "uniqueness",
		});
	}
}

async function updateManagedCinaAuthUser(
	adapter: Pick<DBAdapter, "findOne">,
	...
	if (!user) {
		throw createSCIMError("CONFLICT", {
			detail: "The linked CinaAuth User no longer exists",
		});
	}
	await assertCinaAuthEmailAvailable(adapter, input.email, user.id);
		...
		async (ctx) => {
			...
			const scimUser = await runSCIMCreateWithUniquenessCheck(
					...
					runIdentityMutationTransaction(
						...
						async (trx) => {
							...
							if (resolution.action === "create") {
								await assertCinaAuthEmailAvailable(trx, profile.primaryEmail);
								...
								if (!linkedUser) {
									throw createSCIMError("CONFLICT", {
										detail: "The resolved CinaAuth User does not exist",
									});
								}
								user = linkedUser;
							}
							...
							if (managesProfile) {
								...
								if (resolution.action === "link") {
									await updateManagedCinaAuthUser(
										trx,
				...
				async () => {
					...
					if (resolution.action === "create") {
						await assertCinaAuthEmailAvailable(adapter, profile.primaryEmail);
						return;
					}
			...
			const updatedSCIMUser = await runIdentityMutationTransaction(
				...
				async (trx) => {
					...
					if (subject.profileSourceId === currentSource.id) {
						await updateManagedCinaAuthUser(trx, ctx.context.internalAdapter, {
