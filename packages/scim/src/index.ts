...
import type { CinaAuthPlugin, Status } from "cinaauth";
...
const SCIM_RESPONSE_MARKER = "x-cinaauth-scim-response";
...
function validateConnections(options: SCIMOptions): void {
	...
	for (const connection of options.connections) {
		...
		if (connectionIds.has(connection.id)) {
			throw new CinaAuthError("SCIM connection ids must be unique.");
		}
		...
		for (const credential of connection.credentials) {
			...
			if (
				...
				throw new CinaAuthError("SCIM credential expiry must be a valid Date.");
			}
			if (bearerTokens.has(credential.token)) {
				throw new CinaAuthError("SCIM bearer tokens must be unique.");
			}
...
/**
 * Adds an inbound SCIM 2.0 service provider to CinaAuth.
 *
...
/** The CinaAuth plugin returned by {@link scim}. */
