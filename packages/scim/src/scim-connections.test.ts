...
describe("SCIM connection provisioning", () => {
	...
	it("deactivates a SCIM User without deleting the global CinaAuth User", async () => {
		const data = {
			user: [] as User[],
			session: [] as { id: string }[],
			verification: [] as { id: string }[],
	...
	it("deletes only the SCIM User link and preserves the CinaAuth User", async () => {
		const data = {
			user: [] as User[],
			session: [] as { id: string }[],
			verification: [] as { id: string }[],
