...
export interface GenericOAuthConfig<ID extends string = string> {
	 ...
	 * the provider uses a different immutable identifier. CinaAuth never
	 ...
	 * When set, sign out only clears the CinaAuth session.
	 */
	 ...
	 * Parameters already set by CinaAuth are preserved. Configure token
	 ...
	 * `id_token` to the authorization request by default: CinaAuth sends a
