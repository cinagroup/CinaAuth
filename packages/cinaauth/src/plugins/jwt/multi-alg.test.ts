 ...
 * that skipped `npx @cinaauth/cli generate` after the schema bump.
 * --------------------------------------------------------------------- */
...
describe("unpinned tokens stay on keyPairConfig.alg after lazy-minting extras", () => {
	it("ID token (unpinned) signs with primary alg even after a non-primary alg is lazy-minted", async () => {
		...
		expect(published.keys.map((key) => key.alg).sort()).toEqual([
			"ES256",
			"EdDSA",
		]);
		const stored = await ctx.adapter.findMany<Jwk>({ model: "jwks" });
		expect(stored.every((key) => !key.expiresAt)).toBe(true);
