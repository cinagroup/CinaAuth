	...
	return {
		...
		init(ctx) {
			...
			if (gracePeriod !== undefined && cookieMaxAge > gracePeriod) {
				ctx.logger.warn(
					`[cinaauth] \`session.cookieCache.maxAge\` (${cookieMaxAge}s) exceeds the JWT plugin JWKS grace period (${gracePeriod}s). Rotated keys may stop verifying cookie-cache JWTs before the cookie expires.`,
				);
			}
