		...
		await client.getSession({
			fetchOptions: {
				...
				onSuccess(context) {
					...
					refreshedMaxAge = parsed.get("cinaauth.session_token")?.["max-age"];
				},
			},
		});
...
describe("cookie cache with JWT strategy backed by JWKS", async () => {
	...
	it("should ignore a JWKS cookie cache from a different session token", async () => {
		...
		mixedHeaders.set(
			"cookie",
			`cinaauth.session_data=${firstCookieCache}; cinaauth.session_token=${secondSessionCookie}`,
		);
	...
	it("should not allow tampering with the cookie in JWKS mode", async () => {
		...
		headers.set(
			"cookie",
			`cinaauth.session_data=${tamperedToken}; cinaauth.session_token=${sessionCookie}`,
		);
		const res = await client.getSession({
			fetchOptions: {
				headers,
	...
	it("should reject a secret-signed cookie cache and fall back to the database", async () => {
		...
		const token = cookies.get("cinaauth.session_data");
		...
		headers.set(
			"cookie",
			`cinaauth.session_data=${secretSignedToken}; cinaauth.session_token=${sessionCookie}`,
		);
			...
			fetchOptions: {
				...
				onSuccess(context) {
					replacedCookieCache = parseSetCookieHeader(
						context.response.headers.get("set-cookie") || "",
					).get("cinaauth.session_data")?.value;
				},
			},
		});
