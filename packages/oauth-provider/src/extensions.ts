...
import { CLIENT_ASSERTION_TYPE } from "@cinaauth/core/oauth2";
import type {
	ClientDiscovery,
...
function assertNonEmptyExtensionValue(name: string, value: string) {
	...
	throw new CinaAuthError(`OAuth Provider extension ${name} cannot be empty`);
}
 ...
 * result is shared across two `CinaAuth()` instances) does not register it
