...
async function restoreRegistrationRequestProofContextFromOAuthState(
	...
	const state = (await getOAuthState())?.serverContext;
	...
	return {
		...
		init: async (ctx) => {
			...
			return {
				options: {
					databaseHooks: {
						...registrationProofHooks.databaseHooks,
						session: {
							...registrationProofHooks.databaseHooks.session,
							delete: {
					...
					handler: createAuthMiddleware(async (ctx) => {
						...
						if (isSocialOAuthStartPath(ctx.path)) {
							await addOAuthServerContext({
								query: queryParams.toString(),
								...(parsePrompt(queryParams.get("prompt") ?? "").has(
									"create",
								) &&
								querySignature &&
								queryExpiresAt
									? {
											[registrationProofQuerySignatureStateKey]: querySignature,
											[registrationProofExpiresAtStateKey]:
				...
				"/oauth2/end-session",
				{
					...
					metadata: {
						...
						allowedMediaTypes: [
							...
							// CinaAuth's generated client sends endpoint bodies as JSON.
							"application/json",
						],
						$Infer: {
						...
						openapi: {
							description:
								"RP-Initiated Logout endpoint. Accepts standard logout parameters through GET or an application/x-www-form-urlencoded POST body, plus JSON for CinaAuth generated-client calls. Requests without a usable id_token_hint may require explicit OP confirmation.",
							responses: {
								"200": {
									description:
