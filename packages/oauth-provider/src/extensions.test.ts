...
import { CLIENT_ASSERTION_TYPE } from "cinaauth/oauth2";
import { jwt } from "cinaauth/plugins/jwt";
import { getTestInstance } from "cinaauth/test";
import type { CinaAuthPlugin, User } from "cinaauth/types";
...
describe("oauth-provider extensions", async () => {
	...
	const extensionGrant = "urn:cinaauth:test:grant";
	const extensionOpaqueGrant = "urn:cinaauth:test:opaque-grant";
	const extensionOpaqueBoundGrant = "urn:cinaauth:test:opaque-bound-grant";
	const extensionDriftGrant = "urn:cinaauth:test:drift-grant";
	const extensionAuthMethod = "test_attestation_jwt";
	const extensionAssertionType =
		"urn:cinaauth:test:client-assertion-type:test-attestation";
				...
				grants: {
					[extensionGrant]: async ({ ctx, provider }) => {
						...
						return provider.issueTokens({
							...
							tokenResponse: {
								issued_token_type: "urn:cinaauth:test:access_token",
							},
						});
					},
	...
	it("dispatches extension grants through shared token issuance", async () => {
		...
		expect(tokenResponse.data?.issued_token_type).toBe(
			"urn:cinaauth:test:access_token",
		);
	...
	it("rejects unsupported extension grant types before credential handling", async () => {
		const response = await client.$fetch("/oauth2/token", {
			method: "POST",
			body: new URLSearchParams({
				grant_type: "urn:cinaauth:test:missing-grant",
			}),
			headers: {
				"content-type": "application/x-www-form-urlencoded",
			},
	...
	it("rejects two extensions registering the same grant type", async () => {
		const sharedGrant = "urn:cinaauth:test:shared-grant";
		...
		await expect(
			getTestInstance({
				...
				plugins: [
					...
					makeAuthExtensionPlugin("auth-ext-a", "urn:cinaauth:test:a"),
					makeAuthExtensionPlugin("auth-ext-b", "urn:cinaauth:test:b"),
				],
			}),
	...
	it("rejects two extensions registering the same assertion type", async () => {
		const sharedAssertion = "urn:cinaauth:test:shared-assertion";
	...
	it("authenticateClient binds the assertion audience to the served endpoint", async () => {
			...
			extensions: [
				{
					clientAuthentication: {
						audience_probe_method: {
							assertionTypes: ["urn:cinaauth:test:audience-probe"],
							authenticate: async ({
								expectedAudience,
							}: {
		...
		const ctx = {
			...
			body: {
				...
				client_assertion_type: "urn:cinaauth:test:audience-probe",
			},
