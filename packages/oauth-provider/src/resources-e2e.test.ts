...
/**
 * Boots an in-memory cinaauth instance with the oauth-provider plugin and
...
describe("seed path applies identifier validation and warns on failures", () => {
	...
	beforeEach(() => {
		warnMessages = [];
		// Tap @cinaauth/core/env logger.warn so we can assert seed warns.
		// (CinaAuth's logger forwards to console.warn by default; tap that.)
		warnSpy = vi
			.spyOn(console, "warn")
