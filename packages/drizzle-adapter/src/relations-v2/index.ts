...
function getAffectedRowCount(
	...
	if (typeof count !== "number" || !Number.isFinite(count)) {
		logger.error(
			`[Drizzle Adapter] The result of the ${operation} operation is not a finite number. This is likely a bug in the adapter. Please report this issue to the CinaAuth team.`,
			{ result, ...context },
		);
...
export interface DrizzleAdapterConfig {
	...
	/**
	 * Database schema namespace, used during the CinaAuth CLI to generate the schema.
	 *
...
export const drizzleAdapter = (db: DB, config: DrizzleAdapterConfig) => {
	...
	const createCustomAdapter =
		...
		({ getFieldName, getDefaultModelName, options, schema: baSchema }) => {
			 ...
			 * often plural ("users") even when CinaAuth uses singular model
			...
			function checkMissingFields(
				...
				for (const key in values) {
					if (!schema[key]) {
						throw new CinaAuthError(
							`The field "${key}" does not exist in the "${model}" Drizzle schema. Please update your drizzle schema or re-generate using "npx @cinaauth/cli@latest generate".`,
						);
					}
				}
			}
				...
				async findOne({ model, where, select, join }) {
					...
					if (join) {
						...
						if (!db.query || !queryModel) {
							logger.error(
								`[# Drizzle Adapter]: The model "${model}" was not found in the query object. Please update your Drizzle schema to include relations or re-generate using "npx @cinaauth/cli@latest generate".`,
							);
	...
	adapterOptions = {
		config: {
			...
			// If CinaAuth also pre-stringified, the value would be JSON-encoded twice.
			supportsJSON: true,
