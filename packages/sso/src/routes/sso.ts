import { getCurrentAdapter, runWithTransaction } from "@cinaauth/core/context";
import { isAPIError } from "@cinaauth/core/utils/is-api-error";
import { appendQueryParams } from "@cinaauth/core/utils/url";
import type {
...
export const registerSSOProvider = <O extends SSOOptions>(options: O) => {
	...
	return createAuthEndpoint(
		...
		async (ctx) => {
			...
			if (ctx.context.hasPlugin("scim-legacy")) {
...
export const signInSSO = (options?: SSOOptions) => {
	return createAuthEndpoint(
		...
		async (ctx) => {
			...
			if (
				options?.authorizeProvider &&
				!(await options.authorizeProvider({ provider, flow: "sign-in" }))
			) {
				throw new APIError("FORBIDDEN", {
					message: "SSO provider access is disabled",
				});
			}
...
async function handleOIDCCallback(
	...
	if (
		options?.authorizeProvider &&
		!(await options.authorizeProvider({ provider, flow: "callback" }))
	) {
		throw ctx.redirect(
			`${
				errorURL || callbackURL
			}?error=access_denied&error_description=sso_provider_access_disabled`,
		);
	}
