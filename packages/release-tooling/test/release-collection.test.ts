	...
	const result = spawnSync(
		...
		],
		{
			...
			env: {
				...
				GH_TOKEN: "",
				GITHUB_TOKEN: "",
				GITHUB_REPOSITORY: "cinaauth/cinaauth",
...
function createReleaseWithFollowUpCommit(options: ReleaseFixtureOptions = {}): {
	...
	writeFixtureFile(
		...
		JSON.stringify({ name: "cinaauth", version: "1.0.0" }),
	);
	writeFixtureFile(workspace, "packages/cinaauth/README.md", "# CinaAuth");
	git(workspace, ["add", "."]);
	...
	writeFixtureFile(
		workspace,
		changesetPath,
		["---", '"cinaauth": major', "---", "", changesetDescription].join("\n"),
	);
	...
	writeFixtureFile(
		workspace,
		"packages/cinaauth/package.json",
		JSON.stringify({ name: "cinaauth", version: "2.0.0" }),
	);
	git(workspace, ["add", "."]);
