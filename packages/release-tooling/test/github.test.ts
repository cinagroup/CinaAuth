...
function createFetch(): typeof fetch {
	const request = async (
		...
		if (url.endsWith("/repos/cinaauth/cinaauth/pulls/42")) {
			return jsonResponse({
				number: 42,
				...
				head: {
					ref: "fix/session",
					repo: { full_name: "cinaauth/cinaauth" },
				},
				base: { ref: "main" },
				labels: [{ name: "core" }],
			});
		}
		if (url.includes("/repos/cinaauth/cinaauth/pulls/42/files")) {
			...
			return jsonResponse(
				[
					{
						filename: "packages/cinaauth/src/session.ts",
						status: "modified",
						additions: 1,
						deletions: 1,
						patch: "-oldSession();\n+newSession();",
		...
		if (url.includes("/repos/cinaauth/cinaauth/pulls/404/files")) {
			return jsonResponse([]);
		}
		if (url.endsWith("/repos/cinaauth/cinaauth/pulls/404")) {
			return jsonResponse({ message: "Not Found" }, 404);
		}
		if (url.includes("/repos/cinaauth/cinaauth/pulls/500/files")) {
			return jsonResponse([]);
		}
		if (url.endsWith("/repos/cinaauth/cinaauth/pulls/500")) {
			return jsonResponse({ message: "Server Error" }, 500);
		}
		if (url.endsWith("/repos/cinaauth/cinaauth/releases/tags/v1.2.3")) {
			return jsonResponse({ body: "Release body" });
		}
		if (url.endsWith("/repos/cinaauth/cinaauth/releases/tags/v0.0.0")) {
			return jsonResponse({ message: "Not Found" }, 404);
		}
...
describe("GitHubReader", () => {
	it("aborts GitHub requests after the configured timeout", async () => {
		...
		const github = createGitHubReader({
			repository: parseGitHubRepository("cinaauth/cinaauth"),
			token: "test-token",
	...
	it("maps pull requests and paginated files into release data", async () => {
		...
		await expect(github.getPullRequest(42)).resolves.toEqual({
			...
			isFork: false,
			changedFiles: [
				"packages/cinaauth/src/session.ts",
				"packages/core/src/session.ts",
			],
			diff: [
				"packages/cinaauth/src/session.ts (modified, +1/-1)",
				"-oldSession();",
				"+newSession();",
				"",
	...
	it("rejects ambiguous repository slugs", () => {
		expect(() => parseGitHubRepository("cinaauth")).toThrow(
			"Invalid GitHub repository",
		);
		expect(() => parseGitHubRepository("cinaauth/cinaauth/extra")).toThrow(
			"Invalid GitHub repository",
		);
	});
});
