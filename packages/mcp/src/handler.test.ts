...
import { createInsufficientScopeError } from "cinaauth/oauth2";
...
vi.mock("cinaauth/oauth2", async (importOriginal) => {
	const actual = await importOriginal<typeof import("cinaauth/oauth2")>();
...
describe("createMcpProtectedRequestHandler", () => {
	...
	it("preserves private JWKS transport and its cache identity", async () => {
		const jwksFetch = vi.fn(async () => ({ keys: [] }));
		const jwksCacheKey = {};
		const options = { ...protectedRequestOptions, jwksFetch, jwksCacheKey };
		verifyAccessTokenRequest.mockResolvedValue({ sub: "user-private" });
		const response = await createMcpProtectedRequestHandler(
			options,
			async () => new Response("verified"),
		)(request());
		expect(response.status).toBe(200);
		expect(verifyAccessTokenRequest).toHaveBeenCalledWith(
			expect.anything(),
			expect.objectContaining({ jwksFetch, jwksCacheKey }),
		);
	});
