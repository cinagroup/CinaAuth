...
export async function createAdminAction(opts: unknown) {
	...
	if (typeof createUser !== "function") {
		exitWithError(
			"The admin plugin is required. Add `admin()` to your CinaAuth plugins before running this command.",
		);
	}
	...
	try {
		...
		exitWithError(
			`Failed to inspect existing users. Make sure your database is reachable and your CinaAuth schema is migrated before running this command.${
