...
const compoundIndexPlugin = (): CinaAuthPlugin => ({
...
`,
				...
				try {
					...
					await execFileAsync(process.execPath, args, {
						...
						env: {
							...process.env,
							CINAAUTH_TELEMETRY_DISABLED: "true",
						},
					});
...
`;
		...
		it("reports a required column CinaAuth never writes", async () => {
			...
			expect(schemaProblems?.[0]).toContain(
				'Column "issuer" on table "Account" is required but CinaAuth never writes it',
			);
...
`,
	...
	it("should disambiguate duplicate relations when usePlural is enabled", async () => {
		...
		const schema = await generateDrizzleSchema({
			file: "test.drizzle",
			adapter: database({} as CinaAuthOptions),
			options: {
				database,
				plugins: [testPlugin()],
			},
	...
	it("should emit one() for unique reverse relations", async () => {
		const uniqueProfilePlugin = (): CinaAuthPlugin => ({
			id: "unique-profile",
			schema: {
				profile: {
					fields: {
