...
describe("upgradeAction", () => {
	...
	it("upgrades CinaAuth packages to the running CLI version", async () => {
		...
		expect(mockInstallDependencies).toHaveBeenCalledWith({
			dependencies: ["cinaauth@1.7.0-rc.4", "@cinaauth/core@1.7.0-rc.4"],
			packageManager: "pnpm",
			cwd: process.cwd(),
			type: "prod",
		});
		expect(mockInstallDependencies).toHaveBeenCalledWith({
			dependencies: ["@cinaauth/passkey@1.7.0-rc.4"],
			packageManager: "pnpm",
			cwd: process.cwd(),
			type: "dev",
		});
	...
	it("keeps valid dependencies when another entry is malformed", async () => {
		...
		expect(mockInstallDependencies).toHaveBeenCalledWith({
			dependencies: ["cinaauth@1.7.0-rc.4"],
			packageManager: "pnpm",
			cwd: process.cwd(),
			type: "prod",
		});
	...
	it("upgrades every package in the Changesets fixed release group", async () => {
		...
		const releaseTrainPackages = changesetConfig.fixed
			.find((group) => group.includes("cinaauth"))
	 ...
	 * @see https://github.com/better-auth/better-auth/pull/10760
	 */
	it("does not align independently versioned CinaAuth packages", async () => {
		mockGetPackageInfo.mockReturnValue({
			dependencies: {
