...
describe("info command", () => {
	...
	it("reports installed versions for protocol-based dependencies", async () => {
		...
		await Promise.all([
			writeInstalledPackage("cinaauth", "1.7.2"),
		...
		expect(output.CinaAuth.version).toBe("1.7.2");
	...
	it("reports the installed version when config loading fails", async () => {
		await fs.writeFile(
			...
			JSON.stringify({
				name: "test-project",
				dependencies: { cinaauth: "catalog:" },
			}),
		);
		await writeInstalledPackage("cinaauth", "1.7.2");
