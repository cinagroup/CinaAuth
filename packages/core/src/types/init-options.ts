...
/**
 * What CinaAuth is about to do with an incoming identity when
 * {@link CinaAuthOptions.user}'s `validateUserInfo` runs.
 *
 ...
 * methods cover CinaAuth's built-ins; the open `string` keeps it extensible
...
export type DynamicBaseURLConfig = {
	 ...
	 * If omitted, CinaAuth throws when the request host is unavailable or not
	 * allowed.
	 *
...
export interface CinaAuthRateLimitStorage {
	 ...
	 *
	 * Custom storages must implement this operation directly. CinaAuth no
...
export type CinaAuthAdvancedOptions = {
	...
	database?:
		| {
				 ...
				 * native joins use them; otherwise CinaAuth falls back
				 * to separate queries.
				 *
...
export type CinaAuthOptions = {
	...
	user?:
		| (CinaAuthDBOptions<"user", keyof BaseUser> & {
				/**
				 * Gate which identities CinaAuth admits. Called just before
