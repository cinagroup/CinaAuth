 ...
 */
export function checksSchema(options: CinaAuthOptions): boolean {
	return options.advanced?.database?.validateSchema !== false;
}

/**
 * Resolves when the schema can hold what CinaAuth writes. Returns nothing
...
/** Invalidates cached checks after CinaAuth changes this database's schema. */
 ...
 * left untouched, so this works for adapters CinaAuth does not own.
 */
