...
/**
 * The tables CinaAuth writes, keyed the way the store addresses them:
...
/**
 * Compares the tables CinaAuth writes with what the store holds.
 *
 * A table or column CinaAuth writes must exist. A column CinaAuth does
		...
		case "unexpected-required-column": {
			const issuer =
				finding.column === "issuer"
					? " If this column came from CinaAuth 1.7.0 through 1.7.2, follow the upgrade guide before removing it: https://www.better-auth.com/docs/guides/1-7-upgrade-guide"
					: "";
			return `Column "${finding.column}" on table "${finding.table}" is required but CinaAuth never writes it, so every insert into "${finding.table}" fails. ${relaxHint[source]}${issuer}`;
		}
	}
}
...
function formatSchemaMismatch(
	...
	if (required.length) {
		sections.push(
			`  Required columns CinaAuth never writes\n    ${required.join("\n    ")}`,
		);
		sections.push(
	...
	if (hasIssuer) {
		sections.push(
			"  note: If this column came from CinaAuth 1.7.0 through 1.7.2,\n" +
 ...
 * ```
 */
export class SchemaMismatchError extends CinaAuthError {
