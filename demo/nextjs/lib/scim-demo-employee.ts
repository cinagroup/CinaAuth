import { acquireActiveSCIMUserLink } from "@cinaauth/scim";
import type {
	SSOUserResolution,
...
const SCIM_DEMO_EMPLOYEE_ACCESS_AUDIENCE =
	"cinaauth:scim-demo:employee-access:v2";
const SCIM_DEMO_EMPLOYEE_PORTAL_AUDIENCE =
	"cinaauth:scim-demo:employee-portal:v2";
...
export const SCIM_DEMO_EMPLOYEE_PORTAL_COOKIE =
	"cinaauth.scim_demo_employee_portal";
...
async function getEmployeeTokenKey(): Promise<CryptoKey> {
	const secret = process.env.CINAAUTH_SECRET;
	if (!secret || secret.length < 32) {
		throw new Error(
			"CINAAUTH_SECRET must contain at least 32 characters for SCIM employee access",
		);
	}
...
>(
	...
	const baseURL = process.env.CINAAUTH_URL;
	if (!baseURL) throw new Error("CINAAUTH_URL is required for the SCIM demo");
...
function serializeSCIMDemoEmployeePortalCookie(token: string): string {
	const secure =
		process.env.CINAAUTH_URL?.startsWith("https://") === true ? "; Secure" : "";
