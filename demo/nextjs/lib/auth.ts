...
function isCinaAuthPlugin(plugin: unknown): plugin is CinaAuthPlugin {
	return (
...
if (!isCinaAuthPlugin(dashPlugin)) {
	throw new Error("The Dash integration did not return a CinaAuth plugin");
}
...
const authOptions = {
	...
	plugins: [
					...
					variables: {
						...
						inviteLink:
							process.env.NODE_ENV === "development"
								...
								: `${process.env.CINAAUTH_URL || "https://demo.better-auth.com"}/accept-invitation/${data.id}`,
					},
				});
			},
		}),
		...
		multiSession(),
		oAuthProxy({
			productionURL: process.env.CINAAUTH_URL || "https://demo.better-auth.com",
		}),
		nextCookies(),
		oneTap(),
		stripe({
	  ...
	  -----END CERTIFICATE-----`,
		...
		oauthProvider({
			...
			resources: [
				process.env.CINAAUTH_URL || "https://demo.better-auth.com",
				(process.env.CINAAUTH_URL || "https://demo.better-auth.com") +
					"/api/mcp",
			],
			selectAccount: {
