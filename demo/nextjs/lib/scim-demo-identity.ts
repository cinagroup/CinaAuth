...
const SCIM_DEMO_V2_SUBJECT_CONTEXT = "cinaauth:scim-demo:oidc-subject:v2";
...
function getSCIMDemoIdentitySecret() {
	...
	if (!secret) {
		throw new Error(
			"CINAAUTH_SECRET is required to create SCIM demo identities",
		);
	}
	return secret;
}
...
function getSCIMDemoBaseURL() {
	const value = process.env.CINAAUTH_URL;
	if (!value) throw new Error("CINAAUTH_URL is required for the SCIM demo");
...
export function isSCIMDemoOIDCConfigured() {
	...
	const identitySecret = process.env.CINAAUTH_SECRET;
