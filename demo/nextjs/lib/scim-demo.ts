import type { SCIMScope } from "@cinaauth/scim";
...
export function isSCIMDemoEnabled() {
	return (
		...
		Boolean(process.env.CINAAUTH_URL)
	);
}
...
export function getSCIMDemoBaseURL() {
	...
	if (!value) {
		throw new Error("CINAAUTH_URL is required for the SCIM demo");
	}
	...
	if (url.protocol !== "https:" && !(url.protocol === "http:" && isLoopback)) {
		throw new Error(
			"CINAAUTH_URL must use HTTPS unless the SCIM demo runs on loopback",
		);
	}
	return url.origin;
}
