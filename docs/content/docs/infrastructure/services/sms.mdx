---
icon: infrastructureServicesSMS
sidebarTitle: "SMS"
title: SMS Service
description: CinaAuth Infrastructure provides a managed SMS service for sending OTP codes for phone verification and two-factor authentication. Send verification codes without managing SMS providers.
---
...
The SMS service is included in the `@cinaauth/infra` package:

```ts
import { sendSMS, createSMSSender } from "@cinaauth/infra";
```
...
```ts
import { sendSMS } from "@cinaauth/infra";
...
```ts
import { createSMSSender } from "@cinaauth/infra";

const smsSender = createSMSSender({
  apiKey: process.env.CINAAUTH_API_KEY,
  apiUrl: process.env.CINAAUTH_API_URL,
});
...
```ts
interface SMSConfig {
  apiKey?: string;   // Your CinaAuth Infrastructure API key
...
CINAAUTH_API_URL=https://api.cinagroup.com  # Optional
```
...
## Integration with CinaAuth

When using the `dash()` or `sentinel()` plugins with CinaAuth's phone authentication, SMS messages are automatically sent for:
...
### CinaAuth Phone Plugin Integration

```ts
import { CinaAuth } from "cinaauth";
import { phoneNumber } from "cinaauth/plugins";
