---
icon: infrastructureServicesEmail
sidebarTitle: "Email"
title: Email Service
description: CinaAuth Infrastructure provides a managed transactional email service with pre-built templates for common authentication flows. Send verification emails, password resets, invitations, and more without managing email infrastructure.
---
...
The email service is included in the `@cinaauth/infra` package:

```ts
import { sendEmail, createEmailSender } from "@cinaauth/infra";
```
...
```ts
import { sendEmail } from "@cinaauth/infra";
...
```ts
import { createEmailSender } from "@cinaauth/infra";
...
The email service integrates seamlessly with CinaAuth's authentication flows. Here's a complete example:
