...
title: Dashboard
description: The `dash()` plugin connects your CinaAuth instance to CinaAuth Infrastructure, enabling analytics tracking, activity monitoring, event logging, and admin dashboard APIs.
---

The Dashboard plugin is the core connection between your CinaAuth instance and CinaAuth Infrastructure. It powers the web dashboard with real-time data, tracks user activity, and enables admin APIs.

## What the Dashboard Plugin Enables

Once `dash()` is active, the CinaAuth Infrastructure dashboard gives you:
...
import { dash } from "@cinaauth/infra";
...
| ---------------------- | -------- | -------------------------------------------------------------------------------------------- |
| `apiUrl`               | `string` | CinaAuth Infrastructure API URL. Default: `https://dash.better-auth.com`                     |
| `kvUrl`                | `string` | KV store URL. Default: `https://kv.better-auth.com`                                          |
| `apiKey`               | `string` | Your API key for authentication. Falls back to `CINAAUTH_API_KEY` in env.                    |
...
```ts
dash({
  apiKey: process.env.CINAAUTH_API_KEY,
  activityTracking: {
    enabled: true,
...
This opt-in control plane is for the SCIM 1.7 plugin-managed runtime flow. In CinaAuth 1.7, SCIM supports three connection modes:
...
Use this when the application wants CinaAuth to manage the runtime tenant connection lifecycle for SCIM and pair it with SSO-based identity resolution. It is intended for the new 1.7 flow and complements `scim({ managedConnections })`.
...
When `managedDirectorySync.enabled` is `true`, the dash plugin adds the following models. CinaAuth supplies the primary `id` field for each model.
...
| `userId`               | `string` | Required, indexed        | CinaAuth User receiving the membership.    |
...
**Note**: For **Expo** or **React Native**, import `dashClient` from `@cinaauth/infra/native` (same API) and pair it with `sentinelNativeClient` as described in [Sentinel — Expo and React Native](/docs/infrastructure/plugins/sentinel#expo-and-react-native).
