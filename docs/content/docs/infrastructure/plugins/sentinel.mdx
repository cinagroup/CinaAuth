---
icon: infrastructurePluginsSentinel
...
import { sentinel } from "@cinaauth/infra";
...
| `kvUrl`      | `string`          | CinaAuth identification infrastructure URL. Default: `https://kv.better-auth.com`     |
...
import { sentinelClient } from "@cinaauth/infra/client";
...
| -------------------- | --------- | -------------------------------------------------------------------------- | ----------------------------------------------- |
| `identifyUrl`        | `string`  | `CINAAUTH_KV_URL` or `https://kv.better-auth.com`                          | Base URL for the identify endpoint.             |
...
For **Expo** and **React Native** apps, use `sentinelNativeClient` from `@cinaauth/infra/native` instead of using `@cinaauth/infra/client`.
...
| Option                | Type                            | Default                                                                    | Description                                                                  |
| --------------------- | ------------------------------- | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `identifyUrl`         | `string`                        | `CINAAUTH_KV_URL`, then `https://kv.better-auth.com`                       | Identify endpoint base URL.                                                  |
...
4. **Enable client-side auto-solve** - Use `sentinelClient({ autoSolveChallenge: true })` in the browser, or `sentinelNativeClient({ autoSolveChallenge: true })` in Expo / React Native.
...
```
[Sentinel] Missing CINAAUTH_API_KEY. Security checks may fall back to allow mode.
```
...
1. Verify the correct client plugin: `sentinelClient()` for web (`@cinaauth/infra/client`), or `sentinelNativeClient()` for Expo / React Native (`@cinaauth/infra/native`)
