...
<Steps>
  <Step>
    ...
    <Callout type="info">
      * The TikTok API does not provide email addresses. CinaAuth uses the user's stable `open_id` to create a non-routable `{open_id}@tiktok.placeholder.invalid` address.
