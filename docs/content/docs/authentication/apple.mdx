...
<Steps>
  ...
  <Step>
    ...
    For multiple static audiences (for example a Service ID plus a native bundle ID), prefer `clientId: string[]` or `audience: string[]` instead of setting only `appBundleIdentifier`. When the audience must depend on the request (for example reading an `x-platform` header), use a custom [`verifyIdToken`](/docs/concepts/oauth#verifyidtoken) callback — it receives the request context as its third argument.

    <Callout type="warn">
      Apple emits the `email` claim only on the first authorization; every subsequent sign-in omits it, and Apple provides no user-info endpoint to fetch it later. See [Handling Providers Without Email](/docs/concepts/oauth#handling-providers-without-email) for persistence guidance and the `mapProfileToUser` fallback.
    </Callout>
  </Step>
</Steps>
...
If you previously used the in-page generator from this section, use the [`jose` example above](#configure-the-provider) instead. It uses CinaAuth's async provider configuration to generate the Apple client secret JWT from your Client ID, Team ID, Key ID, and private key.
