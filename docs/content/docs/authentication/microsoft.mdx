...
CinaAuth uses the verified Microsoft Entra `oid` claim as the provider-owned account identifier. `mapProfileToUser` can map local user fields but cannot replace this identifier.

<Callout type="warn">
  Upgrading from CinaAuth 1.6 requires a one-time migration of existing Microsoft account rows from `sub` to `oid`. Follow [Migrate Microsoft account identifiers](/docs/guides/1-7-upgrade-guide#migrate-microsoft-account-identifiers) before accepting production traffic on 1.7.
</Callout>
