...
<Steps>
  <Step>
    ...
    <Callout type="info">
      The Roblox API does not provide email addresses. CinaAuth uses the user's stable `sub` claim to create a non-routable `{sub}@roblox.placeholder.invalid` address.
    </Callout>
  </Step>
