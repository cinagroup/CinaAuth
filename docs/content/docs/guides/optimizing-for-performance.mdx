---
icon: guidesOptimizingForPerformance
sidebarTitle: "Optimize for Performance"
title: Optimizing for Performance
description: A guide to optimizing your CinaAuth application for performance.
---

In this guide, we’ll go over some of the ways you can optimize your application for a more performant CinaAuth app.

## Caching

Caching is a powerful technique that can significantly improve the performance of your CinaAuth application by reducing the number of database queries and speeding up response times.
...
Optimizing database performance is essential to get the best out of CinaAuth.
...
If you're using custom adapters (like Prisma, Drizzle, or MongoDB), you can reduce your bundle size by using `cinaauth/minimal` instead of `cinaauth`. This version excludes Kysely, which is only needed when using direct database connections.

### Usage

Simply import from `cinaauth/minimal` instead of `cinaauth`:
...
<Callout type="warning">
  ...
  * Built-in migrations are not supported. Use external migration tools (or use `cinaauth` if you need built-in migration support)
</Callout>
