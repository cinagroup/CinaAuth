---
icon: guidesDynamicBaseURL
title: Dynamic Base URL
description: Configure CinaAuth for preview deployments, multiple domains, and per-request URL resolution.
---
...
When a request comes in, CinaAuth extracts the host from `x-forwarded-host`, `host` header, or the request URL (in that order), validates it against `allowedHosts`, and uses the matched value to build the request-specific base URL.
...
By default, CinaAuth throws if the incoming host does not match `allowedHosts`. That is usually the safer default because it exposes proxy or deployment mistakes immediately.
...
If you need to share cookies across subdomains, you can enable `crossSubDomainCookies` while still using dynamic base URL. CinaAuth will derive the cookie domain from the resolved host unless you set `domain` explicitly.
