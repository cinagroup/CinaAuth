...
sidebarBadge: "New"
title: Upgrading to CinaAuth 1.7
description: Upgrade CinaAuth from 1.6 to 1.7, including Expo, OAuth, OpenID Connect, MCP, SAML, SCIM, proxy, and custom adapter changes.
---

Most CinaAuth 1.7 changes are additive. Most projects start with one command:

```package-install
npx auth upgrade
```

Upgrade `cinaauth` and every `@cinaauth/*` package together so the CLI and the library stay on 1.7.
...
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Uses CinaAuth at all                                                   | [Before you upgrade](#before-you-upgrade) and [Behind a proxy](#behind-a-proxy) |
...
CinaAuth 1.7.0 through 1.7.2 added a required `issuer` column to the `account` table and a unique index on `issuer` and `accountId`. CinaAuth 1.7.3 removes that requirement. Accounts are recognized by `providerId` and `accountId`, as in 1.6.
  ...
  <Step>
    ...
    <Tabs items={["Postgres", "MySQL and SQL Server", "SQLite", "Prisma and Drizzle", "MongoDB"]}>
      ...
      <Tab value="Prisma and Drizzle">
        ...
        ```bash
        npx @cinaauth/cli@latest generate
        ```
      </Tab>
...
4. When stored ID tokens are unavailable, pause Microsoft sign-in and account linking during the cutover and obtain the mapping from a trusted Microsoft Entra export. CinaAuth cannot derive `oid` from the old account row alone; accepting traffic before the migration can create duplicate accounts.
...
**What to do:** upgrade the `@cinaauth/electron` client and server together. Make sure your app URL scheme is in `trustedOrigins`. Remove the old `disableOriginOverride` option. Review trusted entries with a host, such as `myapp://callback`, because they no longer match lookalike hosts.
...
CinaAuth now keeps the scopes already on the account across re-login and token refresh, using the existing `account.scope` field. This is a behavior fix: no schema change, no backfill, and no action for most projects.

### SIWE derives identity from the signed message

Remove wallet address and chain fields from `authClient.siwe.nonce()` and `authClient.siwe.getNonce()` calls. If you configure a server-side `getNonce`, return an ERC-4361 nonce containing 8 to 250 alphanumeric characters. CinaAuth reads the address and chain from the signed SIWE message.
...
This covers `@cinaauth/oauth-provider`.
...
If you already use `@cinaauth/oauth-provider`, migrate existing `oauthClient` rows as follows:
...
In 1.6, `/oauth2/end-session` accepted only `GET` requests and rejected requests without an `id_token_hint`. In 1.7, the endpoint also accepts form-encoded `POST` requests. For browser navigation without a valid hint, CinaAuth asks the user to confirm before ending the current session. The same browser confirmation is required when the hint refers to a different session than the browser session. API calls receive a protocol error when confirmation is required.

The confirmation flow preserves the existing redirect rule: `post_logout_redirect_uri` must exactly match a registered URI. CinaAuth adds `state` only to that verified redirect.
...
**Watch your date columns.** The `max_age` check reads a session's creation time back as a date. If your custom schema stores session timestamps as text instead of a real date or integer-timestamp type, the check can misread the value and send users into a login loop. Store `user`, `session`, `account`, and `verification` timestamps with a date or timestamp type. The CinaAuth CLI generates the correct type.
...
CinaAuth now refuses HTTP redirects on the server-side OAuth requests it makes: token exchange, token refresh, client credentials, token introspection, and JWKS requests. Conformant OAuth providers answer these endpoints directly and do not redirect.
...
**What to do:** replace `oidcProvider` from `cinaauth/plugins` with `oauthProvider` from `@cinaauth/oauth-provider` and move your configuration across.
...
The MCP plugin moves from `cinaauth` to `@cinaauth/mcp` and uses `@cinaauth/oauth-provider`.

**What to do:**

1. Install `@cinaauth/mcp`, `@cinaauth/cimd`, and the official version 2 MCP client or server package your application needs. Import MCP authorization and protected-request helpers from `@cinaauth/mcp`. Replace the removed in-core client and adapters with `@modelcontextprotocol/client` or `@modelcontextprotocol/server`.
...
**What to do:** confirm each IdP signs assertions, or set `wantAssertionsSigned: false` deliberately for an IdP that cannot. Fix or re-register stored SP metadata that no longer validates. If a custom integration supplies its own RelayState on SAML callbacks, make sure it round-trips the value CinaAuth issued.
...
When `baseURL` uses `allowedHosts`, CinaAuth ignores forwarded headers unless you opt in.
...
Without `baseURL.fallback`, dynamic base URLs fail closed when a request has no usable host or its host does not match `allowedHosts`. Direct server API calls must therefore include request URL or header data that resolves to an allowed public origin, or configure a trusted fallback. CinaAuth rebuilds the base URL, trusted origins, provider URLs, and cookies for the resolved origin on each request; OAuth issuer, discovery, protected-resource, and JWKS URLs follow the same request origin.
...
In 1.6, `deviceAuthorization()` signed a device into the same CinaAuth application and returned a CinaAuth session token from `/device/token`. That standalone flow remains available in 1.7 and does not accept RFC 8707 resource indicators or add OAuth fields to the `deviceCode` table.
...
import { oauthDeviceAuthorizationClient } from "@cinaauth/oauth-provider/client";
...
Magic-link and email-OTP sign-in now treat proven mailbox control as the source of truth for an account whose email had never been confirmed. If that account had an unproven password or other linked accounts, CinaAuth removes all of them and revokes existing sessions before signing the user in.
