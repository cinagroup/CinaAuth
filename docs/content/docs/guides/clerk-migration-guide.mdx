---
icon: guidesClerkMigrationGuide
sidebarTitle: "Clerk Migration Guide"
title: Migrating from Clerk to CinaAuth
description: A step-by-step guide to transitioning from Clerk to CinaAuth.
---

In this guide, we'll walk through the steps to migrate a project from Clerk to CinaAuth — including email/password with proper hashing, social/external accounts, phone number, two-factor data, and more.
...
Before starting the migration process, set up CinaAuth in your project. Follow the [installation guide](/docs/installation) to get started. And go to
...
<Steps>
  ...
  <Step>
    ...
    <Callout type="info">
      **Important for Clerk Migrations**: Clerk uses bcrypt to hash passwords, while CinaAuth uses `scrypt` by default. To ensure migrated users can sign in with their existing passwords, you'll need to configure CinaAuth to use bcrypt for password verification.
    </Callout>
    ...
    Then configure CinaAuth to use bcrypt:
  ...
  <Step>
    ...
    import { Pool } from "pg";
    import { CinaAuth } from "cinaauth";
    import { admin, twoFactor, phoneNumber, username } from "cinaauth/plugins";
    import bcrypt from "bcrypt";
  ...
  <Step>
    ...
    Now that the data is migrated, you can start updating your components to use CinaAuth. Here's an example for the sign-in component:
  ...
  <Step>
    ...
    Replace your Clerk middleware with CinaAuth's middleware:
  ...
  <Step>
    ...
    Once you've verified that everything is working correctly with CinaAuth, you can remove Clerk:
...
[Goodbye Clerk, Hello CinaAuth – Full Migration Guide!](https://www.youtube.com/watch?v=Za_QihbDSuk)

## Wrapping Up

Congratulations! You've successfully migrated from Clerk to CinaAuth.
