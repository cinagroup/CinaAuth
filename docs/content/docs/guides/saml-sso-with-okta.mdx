---
icon: guidesSAMLSSOWithOkta
title: SAML SSO with Okta
description: A guide to integrating SAML Single Sign-On (SSO) with CinaAuth, featuring Okta
---
...
SAML (Security Assertion Markup Language) is an XML-based standard for exchanging authentication and authorization data between an Identity Provider (IdP) (e.g., Okta, Azure AD, OneLogin) and a Service Provider (SP) (in this case, CinaAuth).

In this setup:

* **IdP (Okta)**: Authenticates users and sends assertions about their identity.
* **SP (CinaAuth)**: Validates assertions and logs the user in.up.
...
5. Configure the following settings:
   ...
   * **Single Sign-on URL**: Your CinaAuth callback endpoint (e.g., `http://localhost:3000/api/auth/sso/saml2/sp/acs/sso`). Note: `sso` is your `providerId`
   * **Audience URI (SP Entity ID)**: Your CinaAuth metadata URL (e.g., `http://localhost:3000/api/auth/sso/saml2/sp/metadata`)
   * **Name ID format**: Email Address or any of your choice.

6. Download the IdP metadata XML file and certificate

<Callout type="info">
  **IdP-Initiated SSO**: If you want users to access your app from the Okta dashboard, make sure the **Single Sign-on URL** points to the callback endpoint (`/api/auth/sso/saml2/sp/acs/{providerId}`). CinaAuth automatically handles both SP-initiated and IdP-initiated flows.
</Callout>
