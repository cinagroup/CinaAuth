---
icon: guidesSupabaseMigrationGuide
sidebarTitle: "Supabase Migration Guide"
title: Migrating from Supabase Auth to CinaAuth
description: A step-by-step guide to transitioning from Supabase Auth to CinaAuth.
---

In this guide, we'll walk through the steps to migrate a project from Supabase Auth to CinaAuth.
...
<Steps>
  ...
  <Step>
    ...
    * **[phoneNumber](/docs/plugins/phone-number)**: If you have users who signed up with phone numbers
    ...
    import { CinaAuth } from 'cinaauth';
    import { Pool } from "pg";
    import { admin, anonymous, phoneNumber } from 'cinaauth/plugins';
  ...
  <Step>
    ...
    This will create the necessary tables in the `public` schema of your CinaAuth instance.

    Now that we have the necessary tables in our database, we can run the migration script to migrate the users and accounts from Supabase to CinaAuth.
  </Step>
  ...
  <Step>
    ...
    import { DBFieldAttribute } from 'cinaauth/db';
    import { Pool } from 'pg';
    ...
    const CONFIG = {
       ...
       * Phone-only users need an email for CinaAuth
       * Format: {phone_number}@{tempEmailDomain}
       */
      tempEmailDomain: 'temp.cinagroup.com',
    };
    ...
    /**
     * Validates that the imported CinaAuth instance meets migration requirements
     */
    async function validateAuthConfig() {
      ...
      if (errors.length > 0) {
        console.error('\n🟧 CinaAuth Configuration Errors:\n');
        errors.forEach((err) => console.error(`   ${err}`));
        console.error('\n🟧 Please update your CinaAuth configuration to include:\n');
    ...
    async function migrateFromSupabase() {
      ...
      // Validate CinaAuth configuration
    ...
    async function main() {
      console.log('🚀 Supabase Auth → CinaAuth Migration\n');
    ...
    * `tempEmailDomain`: Temporary email domain for phone-only users. Default: "temp.cinagroup.com"
  </Step>
  ...
  <Step>
    ...
    Run the migration script to migrate the users and accounts from Supabase to CinaAuth.
  ...
  <Step>
    ...
    By default, CinaAuth uses the `scrypt` algorithm to hash passwords. Since Supabase uses `bcrypt`, you'll need to configure CinaAuth to use bcrypt for password verification.
  ...
  <Step>
    ...
    Update your codebase from Supabase auth calls to CinaAuth API.

    Here's a list of the Supabase auth API calls and their CinaAuth counterparts.
...
<Steps>
  <Step>
    ...
    Install the CinaAuth SSO package:
  ...
  <Step>
    ...
    /**
     * Migrates Supabase SSO providers to CinaAuth format
     * 
     * @param providers - Array of exported Supabase SSO providers
     * @param CinaAuthUrl - Your CinaAuth base URL (e.g., https://app.example.com)
     */
    async function migrateSSOProviders(
      ...
      for (const provider of providers) {
        ...
        // Transform Supabase attribute mapping to CinaAuth format.
        // Supabase supports both { name: "attr" } and { names: ["attr1", "attr2"] };
        // CinaAuth only uses a single attribute name, so take the first match.
        ...
        // CinaAuth requires inline metadata XML, not a URL.
        ...
        const samlConfig = {
          ...
          spMetadata: {
            entityID: `${CinaAuthUrl}/api/auth/sso/saml2/sp/metadata?providerId=${providerId}`,
          },
          // Name ID format from Supabase config
          identifierFormat: provider.saml.name_id_format || 
            'urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress',
          // Map Supabase attribute names to CinaAuth fields
          mapping: {
        ...
        const ssoProviderData: SSOProviderInsertData = {
          ...
          organizationId: null, // Set this if linking to a CinaAuth organization
  ...
  <Step>
    ...
    CinaAuth identifies SAML accounts by the actual IdP entity ID from the provider metadata and the signed `NameID`. Supabase exports do not contain enough verified data to derive those values safely. Before migrating accounts, create an operator-reviewed mapping from each legacy SSO identity to its IdP entity ID and signed `NameID`. Do not substitute the service-provider `entity_id`, an email address, or a profile `sub` claim.
  ...
  <Step>
    ...
    This links each migrated account to the protocol identity that CinaAuth will verify at sign-in. Stop the migration when a trusted mapping is missing, resolve it with the identity provider, then rerun the batch.
  </Step>
  ...
  <Step>
    ...
    // Your CinaAuth application URL
    const CINAAUTH_URL = process.env.CINAAUTH_URL || 'https://app.example.com';
    ...
    async function main() {
      ...
      await migrateSSOProviders(providers, CINAAUTH_URL);
    }
  ...
  <Step>
    ...
    After migrating, you need to update your IdP (Okta, Azure AD, Google Workspace, etc.) with the new CinaAuth endpoints.

    **Update these settings in your IdP:**

    | Setting              | Old Value (Supabase)                                      | New Value (CinaAuth)                                                         |
    | -------------------- | --------------------------------------------------------- | ---------------------------------------------------------------------------- |
  ...
  <Step>
    ...
    Replace Supabase SSO authentication calls with CinaAuth:
...
- [ ] Providers migrated to CinaAuth database
- [ ] User accounts linked to SSO providers
- [ ] IdP settings updated (ACS URL, Entity ID)
- [ ] Client code updated to use CinaAuth SSO
...
Congratulations! You've successfully migrated from Supabase Auth to CinaAuth.

CinaAuth offers greater flexibility and more features—be sure to explore the [documentation](/docs) to unlock its full potential.
