---
icon: guidesCreateDatabaseAdapter
title: Create a Database Adapter
description: Learn how to create a custom database adapter for CinaAuth
---

Learn how to create a custom database adapter for CinaAuth using `createAdapterFactory`.
...
Our hope is to allow you to focus on writing database logic, and not have to worry about how the adapter is working with CinaAuth.
...
<Steps>
  <Step>
    ...
    ```ts
    import { createAdapterFactory, type DBAdapterDebugLogOption } from "cinaauth/adapters";
  ...
  <Step>
    ...
    The `config` object is mostly used to provide information about the adapter to CinaAuth.
  ...
  <Step>
    ...
    <Callout>
      Learn more about [database adapters](/docs/concepts/database#adapters).
    </Callout>
  </Step>
</Steps>
...
* `options`: The CinaAuth options.
* `schema`: The schema from the user's CinaAuth instance.
...
<Callout type="warn">
  `consumeOne` is optional. Without it, CinaAuth uses a conditional delete and returns the record only when exactly one row was deleted.
</Callout>
...
<Callout type="warn">
  `incrementOne` is optional. Without it, CinaAuth uses conditional updates with bounded retries. If retries are exhausted, the operation throws and the increment is not recorded. Use native increments for heavily contended counters and security controls such as failed-attempt limits.
</Callout>
...
The `createSchema` method allows the [CinaAuth CLI](/docs/concepts/cli) to [generate](/docs/concepts/cli/#generate) a schema for the database.

parameters:

* `tables`: The tables from the user's CinaAuth instance schema; which is expected to be generated into the schema file.
...
We've provided a test suite via the `@cinaauth/test-utils` package that you can use to test your adapter. It requires you to use `vitest`.

First, install the test utilities:

```bash
npm install -D @cinaauth/test-utils
```

Then create a test file using `testAdapter` and `createTestSuite`:

```ts title="my-adapter.test.ts"
import { testAdapter, createTestSuite } from "@cinaauth/test-utils/adapter";
...
<Callout type="info">
  In v1.4, adapter tests used `runAdapterTest` and `runNumberIdAdapterTest` from `cinaauth/adapters/test`. That export has been removed in v1.5. Use `@cinaauth/test-utils/adapter` instead.
</Callout>

## Config

The `config` object is used to provide information about the adapter to CinaAuth.
...
```ts title="Example"
mapKeysTransformOutput: {
  _id: "id", // We want to replace `_id` (from MongoDB) to `id` (for CinaAuth)
},
```
