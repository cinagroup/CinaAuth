---
icon: guidesAuth0MigrationGuide
sidebarTitle: "Auth0 Migration Guide"
title: Migrating from Auth0 to CinaAuth
description: A step-by-step guide to transitioning from Auth0 to CinaAuth.
---

In this guide, we'll walk through the steps to migrate a project from Auth0 to CinaAuth — including email/password with proper hashing, social/external accounts, two-factor authentication, and more.

<Callout type="warn">
  This migration will invalidate all active sessions. When the [Organization](/docs/plugins/organization) plugin is enabled, the script will also attempt a best-effort migration of Auth0 Organizations, their members, and per-member roles. Invitations and enabled-connection configuration are not migrated and should be reconfigured manually in CinaAuth.
</Callout>
...
<Steps>
  ...
  <Step>
    ...
    import { Pool } from "pg";
    import { CinaAuth } from "cinaauth";
    import { admin, twoFactor, username } from "cinaauth/plugins";
  ...
  <Step>
    ...
    import { generateRandomString, symmetricEncrypt } from "cinaauth/crypto";
    ...
    function mapAuth0RoleToCinaAuthRole(auth0Roles: string[]) {
        if (typeof auth0Roles === 'string') return auth0Roles;
        if (Array.isArray(auth0Roles)) return auth0Roles.join(',');
    }
    // helper function to migrate password from Auth0 to CinaAuth for custom hashes and algorithms
            ...
            try {
                ...
                await ctx.adapter.create({
                    ...
                    data: {
                        ...
                        // if you are enterprise user, you can get the refresh tokens or all the tokensets - auth0Client.users.federatedConnectionsTokensets.list(auth0User.user_id)
                ...
                try {
                    ...
                    for (const member of members) {
                        try {
                            ...
                            const role = mapAuth0RoleToCinaAuthRole(userRoles.map(r => r.name));
                            await ctx.adapter.create({
                                model: "member",
                                data: {
                ...
                try {
                    ...
                    const baseUserData = {
                        ...
                        ...(isAdminEnabled ? {
                            ...
                            role: mapAuth0RoleToCinaAuthRole(auth0User.roles || []),
                        } : {}),
  ...
  <Step>
    ...
    <Callout type="warning">
      ...
      3. Verify the migrated data in CinaAuth before proceeding
  ...
  <Step>
    ...
    By default, CinaAuth uses the `scrypt` algorithm to hash passwords. Since Auth0 uses `bcrypt`, you'll need to configure CinaAuth to use bcrypt for password verification.
  ...
  <Step>
    ...
    Now that the data is migrated, update your components to use CinaAuth. Here's an example for the sign-in component:

    ```tsx title="components/auth/sign-in.tsx"
    import { authClient } from "cinaauth/client";
  ...
  <Step>
    ...
    Replace your Auth0 middleware with CinaAuth's middleware:
  ...
  <Step>
    ...
    Once you've verified everything is working correctly with CinaAuth, remove Auth0:
...
The script includes a basic role mapping function (`mapAuth0RoleToCinaAuthRole`). Customize this function based on your Auth0 roles and CinaAuth role requirements.
...
Now! You've successfully migrated from Auth0 to CinaAuth.
