---
icon: conceptsCookies
title: Cookies
description: Learn how CinaAuth uses cookies, including cookie prefixes, custom cookie attributes, cross-subdomain sharing, secure cookies, and handling Safari ITP with proxies.
---
...
By default, CinaAuth cookies follow the format `${prefix}.${cookie_name}`. The default prefix is "cinaauth". You can change the prefix by setting `cookiePrefix` in the `advanced` object of the auth options.
...
By default, CinaAuth uses the following cookies:
...
If your CinaAuth API is hosted on a different domain than your frontend, Safari may block authentication cookies entirely.
