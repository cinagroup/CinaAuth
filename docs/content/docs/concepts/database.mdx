...
CinaAuth connects to a database to store data. The database will be used to store data such as users, sessions, and more. Plugins can also define their own database tables to store data.

You can pass a database connection to CinaAuth by passing a supported database instance in the database options. You can learn more about supported database adapters in the [Other relational databases](/docs/adapters/other-relational-databases) documentation.

<Callout type="info">
  CinaAuth also works without any database. For more details, see [Stateless Session Management](/docs/concepts/session-management#stateless-session-management).
</Callout>

## CLI

CinaAuth comes with a CLI tool to manage database migrations and generate schema.
...
CinaAuth also provides a `generate` command to generate the schema required by CinaAuth. If you're using a database adapter like Prisma or Drizzle, this command will generate the right schema for your ORM. If you're using the built-in Kysely adapter, it will generate an SQL file you can run directly on your database.
...
<Callout>
  ...
  required by CinaAuth is described below and you can find additional schema
  required by plugins in the plugin documentation.
</Callout>

### Programmatic Migrations

In environments where the CLI isn't available (e.g. Cloudflare Workers, serverless functions), you can run migrations programmatically using `getMigrations` from `cinaauth/db/migration`.
...
Secondary storage in CinaAuth allows you to use key-value stores for managing session data, verification records, rate limiting counters, and other short-lived auth data. This can be useful when you want to offload the storage of intensive records to a high performance storage or even RAM.
...
Then, provide your implementation to the `CinaAuth` function:
...
```package-install
@cinaauth/redis-storage ioredis
```
...
export const auth = CinaAuth({
	...
	secondaryStorage: redisStorage({
		client: redis,
		keyPrefix: "cinaauth:", // optional, defaults to "cinaauth:"
	}),
});
```
...
<Tabs items={["node-redis", "Upstash Redis"]}>
  <Tab value="node-redis">
    ...
    ```typescript title="auth.ts"
    import { CinaAuth, type SecondaryStorage } from "cinaauth";
...
CinaAuth requires the following tables to be present in the database. The types are in `typescript` format. You can use corresponding types in your database.
...
An account represents one authentication method linked to a user. CinaAuth recognizes the provider-side identity by the pair of `providerId` and `accountId`, while `id` identifies the local account row. Use `id` when an account API asks for an `accountId`.
...
export const accountTableFields = [
	...
	},
	{
		name: "accountId",
		type: "string",
		description: "The stable account identifier assigned by the provider",
	},
	{
		name: "providerId",
		type: "string",
...
CinaAuth allows you to customize the table names and column names for the core schema. You can also extend the core schema by adding additional fields to the user and session tables.
...
```ts title="auth.ts"
import { CinaAuth } from "cinaauth";
import { twoFactor } from "cinaauth/plugins";
...
CinaAuth provides a type-safe way to extend the `user` and `session` schemas. You can add custom fields to your auth config, and the CLI will automatically update the database schema. These additional fields will be properly inferred in functions like `useSession`, `signUp.email`, and other endpoints that work with user or session objects.
...
* `input`: Whether CinaAuth accepts the field when creating or updating a record (default: `true`). For user fields, this includes values from API input and `mapProfileToUser`. Set this to `false` for server-owned fields such as `role`.
* `returned`: Whether CinaAuth includes the stored field in response bodies (default: `true`). This does not affect whether input can write the field.
...
CinaAuth by default will generate unique IDs for users, sessions, and other entities.
...
Doing this will disable CinaAuth from generating IDs for any table, and will assume your
database will generate the numeric ID automatically.

When enabled, the CinaAuth CLI will generate or migrate the schema with the `id` field as a numeric type for your database
...
<Callout type="info">
  CinaAuth will continue to infer the type of the `id` field as a `string` for the database, but will
  automatically convert it to a numeric type when fetching or inserting data from the database.

  It's likely when grabbing `id` values returned from CinaAuth that you'll receive a string version of a number,
  this is normal. It's also expected that all id values passed to CinaAuth (eg via an endpoint body) is expected to be a string.
</Callout>

### UUIDs

If you prefer UUIDs for the `id` field, you can set the `advanced.database.generateId` option to `"uuid"`.
By default, CinaAuth will generate UUIDs for the `id` field for all tables, except adapters that use `PostgreSQL` where we allow the
database to generate the UUID automatically.

By enabling this option, the CinaAuth CLI will generate or migrate the schema with the `id` field as a UUID type for your database.
...
Database hooks allow you to define custom logic that can be executed during the lifecycle of core database operations in CinaAuth. You can create hooks for the following models: **user**, **session**, and **account**.
    ...
    user: {
      create: {
        before: async (user, ctx) => {
          ...
          return {
            data: {
              // Ensure to return CinaAuth named fields, not the original field names in your database.
              ...user,
...
If you want to stop the database hook from proceeding, you can throw errors using the `APIError` class imported from `cinaauth/api`.
...
Since CinaAuth version `1.4` we've introduced database joins support.
This allows CinaAuth to perform multiple database queries in a single request, reducing the number of database roundtrips.
...
CinaAuth falls back to making multiple database queries and combining the results.
