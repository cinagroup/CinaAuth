...
Beyond authenticating users, CinaAuth also provides a set of methods to manage users. This includes, updating user information, changing passwords, and more.
...
CinaAuth provides a utility to hard delete a user from your database. It's disabled by default, but you can enable it easily by passing `enabled:true`
...
For added security, you’ll likely want to confirm the user’s intent before deleting their account. A common approach is to send a verification email. CinaAuth provides a `sendDeleteAccountVerification` utility for this purpose.
...
**validateUserInfo**: A gate that decides which identities CinaAuth admits. It fires just before a user is created (`create-user`) or a new provider account is linked (`link-account`), for every authentication method (OAuth, OIDC SSO, SAML SSO, email/password, magic link, email OTP, anonymous, SIWE, phone number, admin-created users, and SCIM), including stateless setups with no persistent database, so policy lives in one place instead of per provider.
...
CinaAuth supports multiple authentication methods through providers such as email and password, Google, or an enterprise identity provider. Each method linked to a user is stored as an account.

An account has a local record ID and a provider identity. `id` identifies the CinaAuth account record and is the value to pass as `accountId` to account-management APIs. The pair of `providerId` and `accountId` identifies the external account: `providerId` names the provider configuration, and `accountId` is the stable identifier that provider assigned. See the [account schema](/docs/concepts/database#account) for the complete set of fields.
...
Account linking is [enabled by default](https://www.cinagroup.com/docs/reference/options#accountlinking) and lets users associate multiple authentication methods with a single account. With CinaAuth, users can connect additional social sign-ons or OAuth providers to their existing accounts if the provider confirms the user's email as verified.
...
By default, when a user signs in with an OAuth provider whose email matches an existing user (and either the provider verified the email or it is in `trustedProviders`), CinaAuth automatically links the OAuth account to that user. Set `disableImplicitLinking: true` to turn this off. With this option enabled:
...
Unlink an account by passing the CinaAuth account record's `id`, which you can obtain from `listAccounts`.
...
If the account does not exist or does not belong to the current user, CinaAuth returns an error. CinaAuth also prevents a user from unlinking their only account unless `allowUnlinkingAll` is `true`.
