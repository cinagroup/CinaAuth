---
icon: conceptsRateLimit
title: Rate Limit
description: Learn how to configure rate limiting in CinaAuth, including IP address detection, IPv6 support, custom rate limit windows, storage backends, error handling, and per-endpoint rules.
---

CinaAuth includes a built-in rate limiter to help manage traffic and prevent abuse. By default, in production mode, the rate limiter is set to:
...
You can easily customize these settings by passing the rateLimit object to the CinaAuth function.
...
In addition to the default settings, CinaAuth provides custom rules for specific paths. For example:
...
By default CinaAuth does not trust comma-separated forwarded IP chains, since
...
Or list your proxies in `trustedProxies`. CinaAuth walks the chain right to
...
<Callout type="info">
  The `migrate` command only works if you're using the built-in Kysely adapter. If you're using Prisma, Drizzle, or another ORM, run `npx auth@latest generate` first, then apply the schema using your ORM's migration tool. For more info, see the [CinaAuth CLI](/docs/concepts/cli).
</Callout>
...
`customStorage.consume` must check and increment in one operation. CinaAuth no longer accepts separate `get` and `set` methods for rate limiting because that shape can allow concurrent requests to pass the same stale counter.

## Handling Rate Limit Errors

When a request exceeds the rate limit, CinaAuth returns the following header:

* `X-Retry-After`: The number of seconds until the user can make another request.

To handle rate limit errors on the client side, you can manage them either globally or on a per-request basis. Since CinaAuth clients wrap over Better Fetch, you can pass `fetchOptions` to handle rate limit errors
