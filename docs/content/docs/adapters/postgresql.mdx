...
title: PostgreSQL
description: Integrate CinaAuth with PostgreSQL.
---
...
Then, you can connect it straight into CinaAuth.
...
In most cases, the default schema is `public`. To have CinaAuth use a
...
The CinaAuth CLI migration system automatically detects your configured `search_path`:
...
<Callout type="info">
  **Verifying your schema configuration:**
  ...
  You can verify which schema CinaAuth will use by checking the `search_path`:
