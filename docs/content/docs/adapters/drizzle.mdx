---
icon: adaptersDrizzle
sidebarTitle: "Drizzle"
title: Drizzle ORM Adapter
description: Integrate CinaAuth with Drizzle ORM.
---
...
To use the Drizzle adapter, you need to install the `@cinaauth/drizzle-adapter` package:
...
The [CinaAuth CLI](/docs/concepts/cli) allows you to generate or migrate
your database schema based on your CinaAuth configuration and plugins.

To generate the schema required by CinaAuth, run the following command:
...
Database joins are useful when CinaAuth needs to fetch related data from multiple tables in a single query.
...
export const user = mysqlTable("user", {
  ...
  // This allows drizzle & cinaauth to still use the original field name,
...
Then when using the CinaAuth CLI, it will generate the schema that looks something like this:
...
`@cinaauth/drizzle-adapter/relations-v2` adapter described below.

## Drizzle Relations v2

The current Drizzle adapter uses Drizzle Relations v1.
To use Drizzle Relations v2, you need to use the `@cinaauth/drizzle-adapter/relations-v2` adapter.
...
Then regenerate your schema using the CinaAuth CLI:
