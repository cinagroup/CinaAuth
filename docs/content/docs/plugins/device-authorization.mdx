...
1. Requesting a device code from the CinaAuth demo server
...
<Callout type="info">
  The CLI login command is a demo feature that connects to the CinaAuth demo server to showcase the device authorization flow in action.
</Callout>
...
<Steps>
  <Step>
    ...
    import { deviceAuthorization } from "cinaauth/plugins"; // [!code highlight]
  ...
  <Step>
    ...
    import { deviceAuthorizationClient } from "cinaauth/client/plugins"; // [!code highlight]
...
| -------------------------------------------------------------------------------- | ------------------------- | ------------------------------------------------------------ | --------------- |
| Sign your own device into the same CinaAuth application                          | CinaAuth session token    | `deviceAuthorization()`                                      | `/device/token` |
...
import {
  ...
  oauthProvider,
} from "@cinaauth/oauth-provider"; // [!code highlight]
...
import { DEVICE_CODE_GRANT_TYPE } from "@cinaauth/oauth-provider";
...
<Callout type="warn">
  Do not call `authClient.device.token` for this OAuth flow. That client method polls `/device/token` and returns a CinaAuth session token. Registered OAuth clients must poll `/oauth2/token`.
</Callout>
...
<Callout type="info">
  Both token paths can coexist. First-party device login keeps using `/device/token`. Registered OAuth clients use `/oauth2/token`; `oauthDeviceAuthorization()` prevents their device codes from being redeemed for a CinaAuth session token.
</Callout>

## First-party session flow

The rest of this page describes the first-party path, where `/device/token` returns a CinaAuth session token.
...
### Polling for a CinaAuth session token

After displaying the user code, poll for the CinaAuth session token. The response's `access_token` field contains a CinaAuth session token, not an RFC 8628 OAuth access token:
...
async function authenticateCLI() {
  console.log("🔐 CinaAuth Device Authorization Demo");
...
**validateClient**: Function to validate client IDs. It receives the requested `clientId` and the current endpoint context, and returns a boolean or `Promise<boolean>`. Production deployments should query an authoritative client registry through `ctx.context.adapter`; do not accept arbitrary client IDs.
