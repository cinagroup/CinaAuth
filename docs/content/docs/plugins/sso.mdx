...
<Steps>
  ...
  <Step>
    ...
    import { sso } from "@cinaauth/sso"; // [!code highlight]
  ...
  <Step>
    ...
    import { ssoClient } from "@cinaauth/sso/client" // [!code highlight]
...
<Callout type="info">
  When you register an OIDC provider, CinaAuth automatically fetches and validates the IdP's [OIDC discovery document](#oidc-discovery). Most endpoint fields are optional — see [OIDC Discovery](#oidc-discovery) for details on auto-discovered fields and possible registration errors.
</Callout>

CinaAuth identifies an OIDC account by the provider ID and the verified `sub` claim. The `mapping` option maps profile fields such as email, name, and image, but it cannot replace the account subject.
...
CinaAuth automatically fetches and validates the provider's [OpenID Connect Discovery Document](https://openid.net/specs/openid-connect-discovery-1_0.html) from:
...
<APIMethod path="/sso/register" method="POST" requireSession note="Minimal OIDC configuration — endpoints are discovered automatically from the issuer.">
  ...
  type registerSSOProvider = {
       ...
       *
       * Must not collide with a configured social provider, an
       * `accountLinking.trustedProviders` entry, or a reserved built-in id
       * (e.g. `credential`). Registration is rejected (422) otherwise, since
       * SSO provider ids share the account-linking provider namespace and a
       * collision could otherwise inherit trust meant for that provider.
       */
...
CinaAuth fills in the following fields by reading the IdP's discovery document (if not explicitly provided):
...
CinaAuth does not follow redirects for server-side OIDC requests. Configure the final canonical URL for the discovery, token, userinfo, and JWKS endpoints.

If one of those endpoints returns a `3xx` response, registration or sign-in fails with `oidc_endpoint_redirect`. This keeps endpoint validation tied to the exact URL that CinaAuth fetches.

#### Why Discovery Can Fail

CinaAuth validates that the IdP's metadata is correct and complete **before** allowing registration. This prevents subtle runtime failures during sign-in or token validation.

<Callout type="info">
  CinaAuth does **not** support implicit-only OIDC flows.
...
| `unsupported_token_auth_method` | The IdP only supports token auth methods that CinaAuth doesn't support                                                              |
...
* CinaAuth automatically performs OIDC discovery at registration time
...
Use `resolveUser` when an OIDC or SAML identity must be mapped to an existing CinaAuth user instead of relying on email matching. The resolver runs on every SSO sign-in after the protocol identity has been verified.
...
The callback receives the active transaction adapter as `context.database`. Keep resolver work database-local and idempotent because a fresh SSO authentication attempt may invoke it again; an already consumed OIDC callback or SAML assertion is not replayed. CinaAuth requires native database transactions and transaction async-context support before calling the resolver. If secondary storage is configured, set `session.storeSessionInDatabase: true` and leave `session.preserveSessionInDatabase` unset or `false` so the database remains the session fallback.
...
CinaAuth identifies a SAML account by the provider ID and the signed `NameID`. The `issuer` field identifies your service provider, while the IdP entity ID comes from `idpMetadata.metadata` or an explicit `idpMetadata.entityID` in a manual configuration. Attribute mappings can populate profile fields, but they cannot replace the signed `NameID` account subject.
...
import { deriveSAMLIdentityProviderEntityID } from "@cinaauth/sso";
...
#### Atomic Membership Capacity Enforcement

Use `withOrganizationMemberProvisioning` when organization membership is
subject to a shared capacity policy. The hook wraps both the final membership
existence check and the insert, so a deployment can hold a database advisory
...
sso({
    organizationProvisioning: {
        withOrganizationMemberProvisioning: async (
            ...
            return withOrganizationMemberCapacityLock(
                organizationId,
                userId,
                provision,
            );
...
The wrapper must invoke `provision()` exactly once when access is allowed and
must keep the lock until that promise resolves. Throwing prevents the member
write. Existing members should be treated as an idempotent no-op rather than
...
Users who sign in through this provider are automatically added to the "Acme
Corp" organization because the provider is explicitly linked to that
organization. Domain-derived assignment from another sign-in method requires
domain verification, a verified stored user email, and one unambiguous
organization match.

<Callout type="info">
  Automatic SSO provisioning does not accept or cancel pending invitations. An
  invited user must complete the existing invitation flow before CinaAuth
  creates an automatic membership for the same organization.
</Callout>

#### Self-Service SSO Dashboard

If you're using [CinaAuth Infrastructure](https://dash.cinagroup.com/sign-in), you get access to a self-service SSO dashboard that simplifies onboarding enterprise customers. Instead of manually exchanging SAML metadata and certificates, organization admins can generate a shareable onboarding link that guides enterprise IT teams through configuring their identity provider.

The dashboard is available at:

```
https://dash.cinagroup.com/[project]/organization/[orgId]/enterprise
```
...
When your IdP rotates its signing certificate, list both the old and the new PEM under `idpMetadata.cert` as an array. CinaAuth accepts SAML responses signed by either, so users keep signing in while the rotation completes.
...
CinaAuth provides flexibility to support both:
...
CinaAuth validates SAML cryptographic algorithms and warns about deprecated ones (SHA-1, RSA 1.5, 3DES) by default.
...
CinaAuth enforces size limits on SAML payloads to protect against denial-of-service attacks via oversized XML.
...
<Callout type="info">
  When using a full URL, make sure it routes to your CinaAuth instance.
</Callout>
...
Domain verification allows your application to trust a new SSO provider after
validating ownership of every associated domain.
...
Domain verification is also required for automatic organization assignment
after a user signs in through another identity provider, such as a social
provider. CinaAuth uses the stored user's verified email and assigns an
organization only when the verified domain maps to exactly one.
...
<Callout type="info">
  With domain verification disabled, CinaAuth does not create organization
  memberships from email-domain matches. Signing in through a provider that is
  explicitly linked to an organization still provisions normally, and
  `signIn.sso` still resolves providers by email or domain.
</Callout>
...
<Steps>
  ...
  <Step>
    ...
    * **Host:** `_cinaauth-token-{your-provider-id}` (**Note:** An underscore is automatically prepended to follow DNS infrastructure subdomain conventions. The `cinaauth-token` part can be customized through the `domainVerification.tokenPrefix` option)
...
CinaAuth derives the ACS URL automatically from your `baseURL` and `providerId`, so no SP-side configuration is required.

Your IdP must include an `AudienceRestriction` for this SP. The accepted audience is the SP entity ID, or the explicit `audience` value in `samlConfig` when you need to match an IdP-specific SP identifier. CinaAuth also checks bearer `Recipient` and response `Destination` values against this provider's ACS endpoints.
...
CinaAuth supports **IdP-initiated SSO flows**, where users access your application directly from their Identity Provider dashboard (e.g., Okta, Azure AD, OneLogin). This is common in enterprise environments where IT admins prefer centralized app access.

**How it works:**

1. User clicks your app icon in the IdP dashboard
2. IdP POSTs SAMLResponse to `/api/auth/sso/saml2/sp/acs/{providerId}`
3. CinaAuth processes the assertion, creates a session, and redirects to your application
...
<Callout type="warn">
  IdP-initiated flows do not carry a CinaAuth-generated RelayState containing a client-side callback URL, so the redirect falls back to the CinaAuth base URL. In split-origin setups (where the app frontend is on a different origin than the CinaAuth server), this lands on the auth server origin and can result in a 404. Configure `idpInitiatedCallbackUrl` globally under the `saml` plugin options or per provider under `samlConfig` (e.g., `https://app.example.com/dashboard`) to return successful sign-ins and validation errors to the frontend.
</Callout>

<Callout type="info">
  If you previously created a manual GET handler for the SAML callback route as a workaround, you can remove it after upgrading. CinaAuth now handles GET requests automatically.
</Callout>

<Callout type="info">
  **Security:** CinaAuth validates all redirect URLs to prevent open redirect attacks. Only relative paths (e.g., `/dashboard`) and URLs matching your configured `trustedOrigins` are allowed. Malicious URLs like `https://evil.com` or protocol-relative URLs (`//evil.com`) are automatically blocked.
</Callout>
	...
	organizationProvisioning: {
		...
		properties: {
			...
			withOrganizationMemberProvisioning: {
				description: "Wrap the final membership check and insert for atomic capacity enforcement.",
				type: "function",
			},
		},
	},
