---
icon: pluginsSIWE
sidebarTitle: "Sign In With Ethereum"
...
## CinaAuth challenge compatibility

Deployments using the CinaAuth challenge and wallet-management protocol should use [SIWE challenge compatibility](./siwe-v2) and import `siwe` from `cinaauth/plugins/siwe-v2`, with `siweClient` from `cinaauth/plugins/siwe-v2/client`. The default entry follows the upstream 1.7 SIWE protocol; these protocols must not be mixed.
