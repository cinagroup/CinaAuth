...
Any time an endpoint in CinaAuth is called that has a valid API key in the headers, you can automatically create a mock session to represent the user by enabling `enableSessionForAPIKeys` option.
...
For other endpoints/methods that don't use API keys, you should utilize CinaAuth's [built-in rate-limiting](/docs/concepts/rate-limit).
...
<Callout type="warn">
  ...
  * Use `enableSessionForAPIKeys: true` and let CinaAuth handle session creation automatically (recommended)
