...
description: A CinaAuth plugin that enables your auth server to serve as an OAuth 2.1 provider.
---

The **OAuth 2.1 Provider** plugin turns your CinaAuth server into an OAuth authorization server. Applications can request user access through the authorization code flow, while services can use client credentials. Add the `openid` scope when clients also need OpenID Connect (OIDC) identity claims.
...
<Steps>
  <Step>
    ...
    import { oauthProvider } from "@cinaauth/oauth-provider"; // [!code highlight]
  ...
  <Step>
    ...
    CinaAuth serves the OAuth Authorization Server metadata and OpenID Connect discovery metadata from the auth handler automatically. If your framework only forwards requests under a catch-all auth route, make sure the issuer metadata URLs reach `auth.handler`.
  ...
  <Step>
    ...
    <Callout type="info">
      ...
      Discovery advertises `none` whenever the provider is enabled because public clients can be created by an authenticated administrator. This is independent of `allowUnauthenticatedClientRegistration`, which controls who may call Dynamic Client Registration rather than whether public clients exist.
    </Callout>
  </Step>
</Steps>
...
import { oauthProviderClient } from "@cinaauth/oauth-provider/client" // [!code highlight]
...
import { createAuthClient } from "cinaauth/client";
import { oauthProviderResourceClient } from "@cinaauth/oauth-provider/resource-client" // [!code highlight]
...
To enable client registration set `allowDynamicClientRegistration: true` in your CinaAuth config.
...
To enable open client registration without a CinaAuth session, additionally set `allowUnauthenticatedClientRegistration: true` in your auth config. Public clients are registered with `token_endpoint_auth_method: "none"`. Confidential clients receive a one-time `client_secret` in the registration response.
...
The [MCP 2026-07-28 spec](https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization/client-registration#application-type-and-redirect-uri-constraints) requires MCP clients to send an appropriate `application_type`. A [Client ID Metadata Document](/docs/plugins/cimd) may omit it; CinaAuth stores that omission as `null` and validates redirects against the safe union of web and native forms.
...
The authorization server accepts requests without `state` for compatibility with OAuth and OpenID Connect, and echoes `state` back when it is provided. CinaAuth's client helpers generate and validate `state` for you.
...
CinaAuth supports Demonstrating Proof of Possession (DPoP) as defined by [RFC 9449](https://datatracker.ietf.org/doc/html/rfc9449). A client can ask for DPoP-bound tokens by registering with `dpop_bound_access_tokens: true`, by sending `dpop_jkt` on the authorization request, or by requesting a resource configured with `dpopBoundAccessTokensRequired: true`.
...
* `refresh_token`: marks the refresh token revoked and deletes the access tokens it minted, so it can no longer issue tokens. Revoking an already-revoked refresh token, including one invalidated by concurrent rotation, returns `200` after invalidating its token family.
...
The endpoint is available to clients registered with `enable_end_session: true`. It accepts logout parameters in a `GET` query or an `application/x-www-form-urlencoded` `POST` body. CinaAuth's generated client sends the same parameters as JSON. A verified `id_token_hint` can end its referenced session immediately. Without a valid hint, CinaAuth asks the user to confirm before ending the current session. The same confirmation is required when the hint refers to a different session than the browser session.

CinaAuth redirects after logout only when `post_logout_redirect_uri` exactly matches a registered URI. It adds `state` only to that verified redirect. An unregistered or query-modified URI does not cause a redirect. Browser navigation receives confirmation, success, and error pages as HTML. API callers receive the protocol response.
...
CinaAuth protects confirmation with origin and CSRF checks plus a short-lived, signed cookie. When a current session exists, the cookie is tied to it. Successful browser logout returns a logged-out page. Session deletion continues through the normal hooks, including Back-Channel Logout delivery to registered relying parties.
...
The OAuth Provider plugin serves this endpoint automatically from the CinaAuth handler. If you do not set a custom issuer, the issuer path is your basePath, such as `/api/auth`.
...
import { oauthProviderOpenIdConfigMetadata } from "@cinaauth/oauth-provider";
import { auth } from "@/lib/auth";
...
The OAuth Provider plugin serves both path-prefixed issuer aliases automatically from the CinaAuth handler:
...
import { oauthProviderAuthServerMetadata } from "@cinaauth/oauth-provider";
import { auth } from "@/lib/auth";
...
Verification can be performed using `verifyAccessTokenRequest` available through the `oauthProviderResourceClient` plugin or `cinaauth/oauth2` package. This is the recommended resource-server API because it verifies the access token and, when the token is DPoP-bound, also verifies the request method, URL, `Authorization: DPoP` scheme, proof key, replay `jti`, and `ath` claim.

When calling the direct `cinaauth/oauth2` API (or `mcpHandler`), provide
`jwksUrl` when the authorization server is reached over public HTTPS. Runtimes
with a private transport can instead pass `jwksFetch`, an async function that
returns the JWKS through that transport, plus a stable `jwksCacheKey` object to
retain key rotation and cache behavior. For example, a Cloudflare Worker can
load JWKS through a Service Binding rather than silently falling back to the
public authorization-server hostname. `oauthProviderResourceClient`
configuration continues to use `jwksUrl` or `remoteVerify`.

With `cinaauth` package:
...
After the page creates and signs in the new account, continue the original
...
`created: true` only selects the continuation branch; it is not proof that an
account was created. The server accepts it only after the request carrying the
signed OAuth query actually creates both the user and its session. That event
issues a one-time proof bound to the query signature, session, and user. Signing
in to an existing account, swapping the query or session, and replaying a
consumed proof cannot clear the create prompt.

The account-creation request must carry the signed OAuth query. The OAuth
Provider client plugin adds it to non-GET requests made from the sign-up page.

Magic Link registration is also supported. Because sending and verifying a
Magic Link happen in separate requests, the provider stores a database-backed,
single-use bridge from the Magic Link challenge to the signed OAuth query. The
final proof is issued only if consuming that exact challenge creates the user
and session.

<Callout type="warn">
  When `secondaryStorage` is configured, `prompt: "create"` proof storage must
  be database-backed with `verification.storeInDatabase: true`, or the
  secondary storage must implement atomic `getAndDelete`. Without either
  guarantee, the server fails closed with `server_error` instead of accepting a
  non-atomic registration continuation.

  Magic Link registration always requires the database-backed option because
  the signed-query bridge spans its send and verify requests. A secondary-only
  configuration fails closed before sending the Magic Link, even when that
  storage implements atomic `getAndDelete`.
</Callout>
...
For `shouldRedirect`, the setup redirect carries a signed marker bound to the
current session. Call `oauth2.continue({ created: true })` after the setup step;
the server re-evaluates `shouldRedirect` against live state and redirects back
to setup if registration is still incomplete.
...
#### Cloudflare Access email compatibility

[Cloudflare Access Generic OIDC](https://developers.cloudflare.com/cloudflare-one/integrations/identity-providers/generic-oidc/) reads the user's email from the ID token. Configure its scopes as `openid email profile` and its email claim name as `email`. Requesting the scope alone supplies the email through UserInfo; explicitly add the claim to the ID token for this integration:
...
The production CinaAuth Worker includes this configuration. It preserves the user's actual verification status and omits both claims when `email` is not granted, including when a refresh request reduces the scope. Without it, Access can report “User email was not returned” even when the `email` scope is configured correctly.
...
```ts
import { consumeClientAssertion } from "@cinaauth/oauth-provider";
...
<Callout type="warn">
  These rate limits only apply when CinaAuth's global rate limiting is enabled. By default, rate limiting is only enabled in production. See [Rate Limiting](/docs/concepts/rate-limit) for global configuration.
</Callout>
...
<Callout type="info">
  Setting an endpoint to `false` removes the OAuth Provider's stricter per-endpoint limit. The endpoint will still be subject to CinaAuth's global rate limiting if enabled.
</Callout>
...
CinaAuth advertises `acr_values_supported: ["0"]`. In OIDC Core, `"0"` means the authentication did not meet ISO/IEC 29115 level 1. Custom ACR policies are not currently supported. Because `acr_values` is voluntary, requests for other classes continue and the ID token reports `acr: "0"`. In an OpenID Connect request, an essential `claims.id_token.acr` request fails when its `value` or `values` does not include `"0"`.
...
Use the [`@cinaauth/mcp` plugin](/docs/plugins/mcp) when an MCP server is one of your protected resources. It builds on this OAuth Provider and adds MCP defaults, RFC 9728 protected resource metadata, and route helpers that return the authorization challenge MCP clients expect.

`mcp()` is the OAuth Provider for that CinaAuth instance, so do not register both `mcp()` and `oauthProvider()`. It accepts the OAuth Provider options directly. Use `requireMcpAuth` when the MCP route shares the auth instance, or `createMcpProtectedRequestHandler` when the resource server runs separately.
