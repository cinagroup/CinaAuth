---
icon: pluginsSCIMGroupsAndRoles
...
A Group requires a `displayName` and may include an `externalId`. Members reference SCIM User IDs from the same connection. They do not reference CinaAuth User IDs.
...
| `reconcileUser` | Applies the complete desired SCIM state for one CinaAuth User in one provisioning domain. |
...
| `userId`               | The linked CinaAuth User ID.                                             |
