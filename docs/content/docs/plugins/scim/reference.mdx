...
This reference describes the SCIM operations and configuration supported by `@cinaauth/scim`.
...
Use `managedConnections` when CinaAuth should persist runtime tenant connections and issue their bearer credentials. This mode needs no code-defined connection and does not add fields to the CinaAuth User, Organization, Account, or SSO models.
...
The SCIM base path is `/scim/v2` under your CinaAuth base URL. User and Group endpoints require `Authorization: Bearer <token>`. Discovery endpoints are public.
...
| Method   | Path                     | Result                                                                              |
| -------- | ------------------------ | ----------------------------------------------------------------------------------- |
| `POST`   | `/scim/v2/Users`         | Creates a SCIM User and creates or explicitly links a CinaAuth User. Returns `201`. |
...
| `DELETE` | `/scim/v2/Users/:userId` | Deletes the SCIM resource while preserving the CinaAuth User. Returns `204`.        |

Deleting a User also removes its direct Group memberships and projected grants, updates the affected Groups, and reconciles lifecycle and access state. When `externalId` is present, the plugin preserves the link needed to attach a later reprovisioned resource to the same CinaAuth User.
...
A profile-managing source writes the primary email and display name to the CinaAuth User. The email must be globally unique, or the request returns `409 Conflict`. Changing the email clears `emailVerified` because provisioning does not verify mailbox ownership.

The Enterprise User extension supports `employeeNumber`, `costCenter`, `organization`, `division`, `department`, and `manager`. A manager accepts a nonempty identifier string, an RFC object containing `value`, `$ref`, or both, or a singleton array containing that object. CinaAuth accepts a client-supplied read-only `displayName` for provider compatibility but does not persist, echo, or pass it to identity callbacks. Responses and callbacks normalize every retained manager to an object with at least `value` or `$ref`. The manager is an external reference: CinaAuth does not require a matching local User or search another connection.
...
Use this callback to return an explicit create-or-link decision for an incoming User. A `link` result for a missing CinaAuth User returns `409 Conflict`.
...
Receives the complete desired access state for one CinaAuth User in one provisioning domain. The callback runs inside the SCIM transaction and must be idempotent.
...
| `scimUser`              | Stores canonical User attributes and the linked CinaAuth User ID.            |
| `scimGroup`             | Stores canonical Group attributes.                                           |
| `scimGroupMember`       | Stores direct Group-to-User memberships.                                     |
| `scimSubject`           | Coordinates linked SCIM sources and profile authority for one CinaAuth User. |
...
CinaAuth supplies the primary `id` field for each model. The SCIM models also reference the core `user` model where required.
