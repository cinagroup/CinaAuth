...
import { haveIBeenPwned } from "cinaauth/plugins" // [!code highlight]
...
Use `isPasswordCompromised` to check passwords in server-side flows that do not use CinaAuth endpoints.

```ts
import { isPasswordCompromised } from "cinaauth/plugins/haveibeenpwned"
...
```ts title="auth.ts"
import { CinaAuth } from "cinaauth"
import { haveIBeenPwned } from "cinaauth/plugins"
