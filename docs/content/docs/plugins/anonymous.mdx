---
icon: pluginsAnonymous
title: Anonymous  
description: Anonymous plugin for CinaAuth.
---
...
<Steps>
  <Step>
    ...
    import { anonymous } from "cinaauth/plugins" // [!code highlight]
  ...
  <Step>
    ...
    import { anonymousClient } from "cinaauth/client/plugins" // [!code highlight]
...
<APIMethod path="/sign-in/anonymous" method="POST">
  ```ts
...
```ts title="auth.ts"
import { CinaAuth } from "cinaauth"
import { anonymous } from "cinaauth/plugins"
...
<APIMethod path="/delete-anonymous-user" method="POST" noResult>
  ```ts
