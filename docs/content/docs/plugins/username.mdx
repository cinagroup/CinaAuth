...
<Steps>
  <Step>
    ...
    import { username } from "cinaauth/plugins" // [!code highlight]
  ...
  <Step>
    ...
    import { usernameClient } from "cinaauth/client/plugins" // [!code highlight]
...
By default, the plugin exposes an endpoint `/is-username-available` to check if a username is available. You can disable this endpoint by providing `disabledPaths` option to the cinaauth configuration. This is useful if you want to protect usernames from being enumerated.
...
The plugin requires 2 fields to be added to the user table (the `displayUsername` field is omitted when [`displayUsername: false`](#disable-display-username) is set):
