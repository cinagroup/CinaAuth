---
icon: pluginsTestUtils
title: Test Utils
description: Testing utilities for integration and E2E testing
---

The Test Utils plugin provides helpers for writing integration and E2E tests against CinaAuth. It includes factories, database helpers, authentication helpers, and OTP capture functionality.
...
<Steps>
  <Step>
    ...
    import { testUtils } from "cinaauth/plugins" // [!code highlight]
...
CinaAuth infers plugin helpers best from statically defined plugin arrays. If you conditionally spread `testUtils()` into `plugins`, TypeScript can stop inferring `ctx.test` correctly.
...
* `name` - Cookie name (e.g., "cinaauth.session\_token")
* `value` - Cookie value
...
```ts title="auth.test.ts"
import { CinaAuth } from "cinaauth"
import { testUtils, emailOTP } from "cinaauth/plugins"
...
import type { TestHelpers } from "cinaauth/plugins"
