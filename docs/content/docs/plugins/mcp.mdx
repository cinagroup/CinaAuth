...
icon: pluginsMCP
title: MCP
description: Turn your CinaAuth server into an OAuth provider for MCP clients
---
...
`mcp()` configures the OAuth provider with MCP resource binding and serves the [RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728) protected resource metadata. For the MCP 2026-07-28 profile, compose it with [Client ID Metadata Documents](/docs/plugins/cimd) and select the profile that pins CIMD draft-00. MCP deprecates Dynamic Client Registration (DCR), so CinaAuth never enables DCR implicitly.

<Callout>
  Install `@cinaauth/mcp`, the recommended `@cinaauth/cimd` companion package, and version 2 of the official MCP TypeScript SDK.
</Callout>
...
<Steps>
  <Step>
    ...
    ```package-install
    @cinaauth/mcp @cinaauth/cimd @modelcontextprotocol/server zod
    ```
  </Step>
  ...
  <Step>
    ...
    import { jwt } from "cinaauth/plugins"; // [!code highlight]
    import { cimd } from "@cinaauth/cimd"; // [!code highlight]
    import { mcp } from "@cinaauth/mcp"; // [!code highlight]
    import { fetchClientMetadataResource } from "@cinaauth/cimd/node"; // [!code highlight]
...
<Callout type="info">
  CinaAuth owns OAuth discovery, client registration and discovery, PKCE authorization, resource binding, token issuance and refresh, and protected-resource challenges. The official MCP TypeScript SDK v2 owns the stateless MCP protocol and transport. Configure servers with `legacy: "reject"`, and pin SDK client version negotiation to `2026-07-28` when adopting this profile.
</Callout>
...
import { oauthDeviceAuthorization } from "@cinaauth/oauth-provider";
...
The registration endpoint is absent from discovery unless DCR is enabled. A DCR request does not need CinaAuth's `resources` extension: `mcp()` adds its canonical resource as a server-owned default.
...
import { requireMcpAuth } from "@cinaauth/mcp"; // [!code highlight]
...
`requireMcpAuth` verifies tokens against your CinaAuth server's JWKS, so the MCP route can run anywhere as long as it can reach that JWKS URL. When the resource server runs separately from the authorization server, or uses a dynamic `baseURL`, use `createMcpProtectedRequestHandler` with explicit verification options instead of `requireMcpAuth`.

```ts title="mcp-server.ts"
import { createMcpProtectedRequestHandler } from "@cinaauth/mcp"; // [!code highlight]
