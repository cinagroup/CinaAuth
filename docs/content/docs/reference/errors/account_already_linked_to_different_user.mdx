...
another user in your project. To prevent account takeover, CinaAuth blocks the link and
throws this error.
...
* You rely on email matching to decide linking, but the actual account key is `providerId` and
...
* Inspect your `account` database table. Each row points to a `userId` and is keyed by
  `providerId` and `accountId`. For OpenID Connect providers, `accountId` is the verified subject.
...
* Ensure you request stable user identifiers from the provider, such as the OpenID Connect `openid`
  scope, so `accountId` remains consistent across sessions.
