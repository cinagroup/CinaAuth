---
icon: referenceInstrumentation
sidebarTitle: "Instrumentation"
title: Instrumentation (Experimental)
description: Distributed tracing for CinaAuth
---
...
CinaAuth is instrumented through [OpenTelemetry](https://opentelemetry.io/) for distributed tracing. Spans are emitted for endpoints, hooks, database operations and the plugin lifecycle.
The goal of this instrumentation is to provide better insights into the behavior of your CinaAuth instance, helping you debug issues and optimize performance.

## Setup

CinaAuth uses the OpenTelemetry API to create spans. For traces to be collected and exported, at minimum, you must configure a **TracerProvider** and a **SpanExporter** in your application.
...
The following spans are supported by CinaAuth:
...
* `cinaauth.operation_id` - The operation identifier (e.g `getSession`)
...
* **Attributes:**
  * `cinaauth.hook.type` - The hook type (e.g. `before` or `after`)
  * `cinaauth.context` - Execution context: `"user"` for global hooks, `"plugin:{pluginId}"` for plugin-registered hooks
  * `http.route` - The **route template** (low cardinality)
  * `cinaauth.operation_id` - The operation identifier (e.g `getSession`)
...
* `cinaauth.hook.type` - The hook type: `"onRequest"`, `"onResponse"`, or `"middleware"`
* `cinaauth.context` - Execution context: `"plugin:{pluginId}"` (e.g. `plugin:bearer`, `plugin:test-plugin`)
...
* **Operations:** `create`, `update`, `updateMany`, `delete` (used for both single delete and `deleteMany` calls)
* **Attributes:**
  * `cinaauth.hook.type` - The hook type (e.g. `create.before`, `create.after`, `update.before`)
  * `cinaauth.context` - Execution context: `"user"` or `"plugin:{pluginId}"`
  * `db.collection.name` - The database collection/model name (e.g. `user`)
