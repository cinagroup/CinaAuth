---
icon: integrationsNext
sidebarTitle: "Next"
title: Next.js integration
description: Integrate CinaAuth with Next.js.
---

CinaAuth can be easily integrated with Next.js. Before you start, make sure you have a CinaAuth instance configured. If you haven't done that yet, check out the [installation](/docs/installation).
...
<Callout type="info">
  You can change the path on your cinaauth configuration but it's recommended to keep it as `/api/auth/[...all]`
</Callout>
...
import { createAuthClient } from "cinaauth/react" // make sure to import from cinaauth/react
...
The `api` object exported from the auth instance contains all the actions that you can perform on the server. Every endpoint made inside CinaAuth is invocable as a function. Including plugins endpoints.
...
<Callout type="info">
  **Migration from middleware:** Rename `middleware.ts` → `proxy.ts` and `middleware` → `proxy` function. All CinaAuth methods work identically.
</Callout>
...
```ts title="middleware.ts"
import { getCookieCache } from "cinaauth/cookies";
...
CinaAuth is fully compatible with Next.js 16. The main change is that "middleware" is now called "proxy". See the [Auth Protection](#auth-protection) section above for Next.js 16+ proxy examples.
...
All CinaAuth methods work identically. See the [Next.js migration guide](https://nextjs.org/docs/app/api-reference/file-conventions/proxy#migration-to-proxy) for details.
