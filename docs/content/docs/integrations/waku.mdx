---
icon: integrationsWaku
sidebarTitle: "Waku"
title: Waku Integration
description: Integrate CinaAuth with Waku.
---

CinaAuth can be easily integrated with Waku. Before you start, make sure you have a CinaAuth instance configured. If you haven't done that yet, check out the [installation](/docs/installation).

## Create auth instance

Create a file named `auth.ts` in your application. Import CinaAuth and create your instance.
...
<Callout type="info">
  You can change the path on your cinaauth configuration but it's recommended to keep it as `src/pages/_api/api/auth/[...route].ts`
</Callout>
      ...
      after: [
        {
          ...
          handler: createAuthMiddleware(async (ctx) => {
            ...
            if (returned instanceof Headers) {
              ...
              contextData.CinaAuthSetCookie = setCookieHeader;
            }
          }),
        },
      ],
...
See below for the middleware to create to add the `contextData.CinaAuthSetCookie` cookies to the response.
...
You can use the `getSessionCookie` helper from CinaAuth for this purpose:
...
const authMiddleware: () => MiddlewareHandler = () => {
  return async (c, next) => {
    ...
    await next();
    const contextData = getContextData();
    const CinaAuthSetCookie = contextData.CinaAuthSetCookie as
      | string
      | undefined;
    if (CinaAuthSetCookie) {
      c.header("set-cookie", CinaAuthSetCookie, { append: true });
    }
  };
};
