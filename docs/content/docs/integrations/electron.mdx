---
icon: integrationsElectron
sidebarTitle: "Electron"
title: Electron Integration
description: Integrate CinaAuth with Electron.
---

Electron is a popular framework for building cross-platform desktop applications using web technologies.

CinaAuth can be integrated into Electron apps to provide secure authentication flows, leveraging the system browser.

## Installation

<Steps>
  <Step>
    ### Configure a CinaAuth front- & back-end

    Before integrating with Electron, ensure you have a CinaAuth server and client set up.

    To get started, check out our [installation](/docs/installation) guide for setting up CinaAuth.
  </Step>

  <Step>
    ### Install the required packages

    Install the CinaAuth server package and the Electron integration package in your server, Electron app, and web client projects.

    **In each project, run:**

    ```package-install
    cinaauth @cinaauth/electron
    ```
  ...
  <Step>
    ### Add the Electron plugin to your CinaAuth server
  ...
  <Step>
    ...
    export const authClient = createAuthClient({
      baseURL: "http://localhost:8081", // Base URL of your CinaAuth frontend
      plugins: [
        electronClient({
  ...
  <Step>
    ...
    To enable this, you need to add your app's protocol scheme to the `trustedOrigins` on your **CinaAuth server**.
    ...
    Then, update your CinaAuth config to include the scheme in `trustedOrigins`:
  ...
  <Step>
    ...
    <Callout type="warning">
      To run your process in `sandbox` mode, you need to ensure that `@cinaauth/electron` is bundled into the preload script.
      Import only `@cinaauth/electron/preload` inside the preload script with tree-shaking enabled, to avoid bundling unwanted dependencies.
        ...
        preload: {
          build: {
            externalizeDeps: { // [!code highlight]
              exclude: ["@cinaauth/electron"], // [!code highlight]
            }, // [!code highlight]
          },
        },
      });
      ...
      If your config uses a list of externals instead, do **not** add `@cinaauth/electron` to that list so it gets bundled.
    </Callout>
  </Step>
  ...
  <Step>
    ...
    import { setupRenderer } from "@cinaauth/electron/preload";
...
To handle already signed-in users, you can use the `electron.transferUser` method on the client and `electronTransferUser` on the server to transfer the user to the Electron app. [See Example](https://github.com/cinagroup/cinaauth/blob/main/apps/account-portal/app/\(auth\)/sign-in/_components/electron.tsx)
...
The prefix to use for cookies set by the plugin. (defaults to `cinaauth`)
...
Prefix for local storage keys. (defaults to `cinaauth`)
...
Prefix(es) for server cookie names to filter. (defaults to `cinaauth`)

This is used to identify which cookies belong to cinaauth to
...
```ts title="electron/auth-client.ts"
electronClient({
  cookiePrefix: ["cinaauth", "my-app"],
});
```

#### `channelPrefix?`

Channel prefix for IPC bridges. (defaults to `cinaauth`)
