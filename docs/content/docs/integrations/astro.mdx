---
icon: integrationsAstro
sidebarTitle: "Astro"
title: Astro Integration
description: Integrate CinaAuth with Astro.
---

CinaAuth comes with first class support for Astro. This guide will show you how to integrate CinaAuth with Astro.
...
To enable CinaAuth to handle requests, we need to mount the handler to a catch all API route. Create a file inside `/pages/api/auth` called `[...all].ts` and add the following code:
...
declare namespace App {
    ...
    interface Locals {
        user: import("cinaauth").User | null;
        session: import("cinaauth").Session | null;
    }
}
```
