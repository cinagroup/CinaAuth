---
icon: integrationsExpo
sidebarTitle: "Expo"
title: Expo Integration
description: Integrate CinaAuth with Expo.
---

Expo is a popular framework for building cross-platform apps with React Native. CinaAuth supports both Expo native and web apps.
...
<Steps>
  <Step>
    ## Configure A CinaAuth Backend

    Before using CinaAuth with Expo, make sure you have a CinaAuth backend set up. You can either use a separate server or leverage Expo's new [API Routes](https://docs.expo.dev/router/reference/api-routes) feature to host your CinaAuth instance.

    To get started, check out our [installation](/docs/installation) guide for setting up CinaAuth on your server. If you prefer, check out the [full example](https://github.com/cinagroup/cinaauth/examples/tree/main/expo-example).

    To use the new API routes feature in Expo to host your CinaAuth instance you can create a new API route in your Expo app and mount the CinaAuth handler.

    ```ts title="app/api/auth/[...auth]+api.ts"
    import { auth } from "@/lib/auth"; // import CinaAuth handler
  ...
  <Step>
    ...
    Install both the CinaAuth package and Expo plugin into your server application.

    ```package-install
    cinaauth @cinaauth/expo
    ```
  ...
  <Step>
    ...
    * You also need to install both the CinaAuth package and Expo plugin into your Expo application.
  ...
  <Step>
    ...
    Add the Expo plugin to your CinaAuth server.
  ...
  <Step>
    ...
    To initialize CinaAuth in your Expo app, you need to call `createAuthClient` with the base URL of your CinaAuth backend. Make sure to import the client from `/react`.
    ...
    You need to also import client plugin from `@cinaauth/expo/client` and pass it to the `plugins` array when initializing the auth client.
    ...
    * **Secure Cookie Management:** stores cookies securely and automatically adds them to the headers of your auth requests.
    ...
    export const authClient = createAuthClient({
        baseURL: "http://localhost:8081", // Base URL of your CinaAuth backend.
        plugins: [
            expoClient({
                scheme: "myapp",
                storagePrefix: "myapp",
  ...
  <Step>
    ...
    CinaAuth uses deep links to redirect users back to your app after authentication. To enable this, you need to add your app's scheme to the `trustedOrigins` list in your CinaAuth config.
    ...
    Then, update your CinaAuth config to include the scheme in the `trustedOrigins` list.
  ...
  <Step>
    ...
    CinaAuth relies on `package.json` exports to resolve its modules. Starting with **Expo SDK 53+** (including SDK 55), package exports support is enabled by default in Metro, so **no extra Metro configuration is needed**.
    ...
    // Do NOT set this to false — CinaAuth requires package exports
...
With CinaAuth initialized, you can now use the `authClient` to authenticate users in your Expo app.
...
<Accordions>
  <Accordion title="Example: Google Sign-In with `@react-native-google-signin/google-signin`">
    This example shows how to implement Google Sign-In using an idToken. No additional configuration is needed in CinaAuth. However, since this approach uses native code, you'll need to configure Expo to obtain the idToken.

    Follow the [Expo Google authentication guide](https://docs.expo.dev/guides/google-authentication/) to set up `@react-native-google-signin/google-signin`, then pass the idToken to CinaAuth:
...
CinaAuth provides a `useSession` hook to access the current user's session in your app.
...
const makeAuthenticatedRequest = async () => {
  const cookies = await authClient.getCookie(); // [!code highlight]
  const headers = {
...
**scheme**: scheme is used to deep link back to your app after a user has authenticated using oAuth providers. By default, CinaAuth tries to read the scheme from the `app.json` file. If you need to override this, you can pass the scheme option to the client.
...
**cookiePrefix**: Prefix(es) for server cookie names to identify which cookies belong to cinaauth. This prevents infinite refetching when third-party cookies are set. Can be a single string or an array of strings to match multiple prefixes. Defaults to `"cinaauth"`.
...
const authClient = createAuthClient({
    ...
    plugins: [
        expoClient({
            storage: SecureStore,
            // Multiple prefixes
            cookiePrefix: ["cinaauth", "my-app", "custom-auth"]
        })
    ]
});
```
