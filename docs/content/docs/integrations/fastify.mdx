---
icon: integrationsFastify
sidebarTitle: "Fastify"
title: CinaAuth Fastify Integration Guide
description: Learn how to seamlessly integrate CinaAuth with your Fastify application.
---

This guide provides step-by-step instructions for configuring both essential handlers and CORS settings.

<Callout type="important">
  A configured CinaAuth instance is required before proceeding. If you haven't set this up yet, please consult our [Installation Guide](/docs/installation).
</Callout>
...
Configure CinaAuth to process authentication requests by creating a catch-all route:

```ts title="server.ts"
import Fastify from "fastify";
import { fromNodeHeaders } from "cinaauth/node";
import { auth } from "./auth"; // Your configured CinaAuth instance
...
To retrieve the user's session in your Fastify routes, use the `auth.api.getSession` method. CinaAuth provides a `fromNodeHeaders` helper function that converts Node.js request headers to the format expected by CinaAuth.
