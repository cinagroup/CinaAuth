---
icon: integrationsLynx
sidebarTitle: "Lynx"
title: Lynx Integration
description: Integrate CinaAuth with Lynx cross-platform framework.
---

This integration guide is for using CinaAuth with [Lynx](https://lynxjs.org), a cross-platform rendering framework that enables developers to build applications for Android, iOS, and Web platforms with native rendering performance.
...
Install CinaAuth and the Lynx React dependency:

```package-install
cinaauth @lynx-js/react
```

## Create Client Instance

Import `createAuthClient` from `cinaauth/lynx` to create your client instance:

```ts title="lib/auth-client.ts"
import { createAuthClient } from "cinaauth/lynx"
...
The Lynx client provides the same API as other CinaAuth clients, with optimized integration for Lynx's reactive system.
...
import { useStore } from "cinaauth/lynx"
import { authClient } from "../lib/auth-client"
...
The Lynx client supports all CinaAuth plugins:
...
Error handling works the same as other CinaAuth clients:
...
* **Full API Compatibility**: All CinaAuth methods and plugins work seamlessly
* **TypeScript Support**: Full type safety with TypeScript inference

The Lynx integration maintains all the features and benefits of CinaAuth while providing optimal performance and developer experience within Lynx's cross-platform ecosystem.
