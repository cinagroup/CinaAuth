...
We've put a lot into CinaAuth 1.7. It's a big release, and some of it means migration work. We wanted to support a wider range of login flows, make the system more robust, and ship security fixes that needed structural changes.

Here's what this release brings.

**Your app can be the login provider for other apps.** If other apps sign in through your app, CinaAuth now handles more OAuth and OpenID Connect rules. This includes DPoP tokens, single sign-out, forced re-login, per-API token rules, and safer token checks.

**More login providers work.** CinaAuth now supports login setups that did not work well before: Amazon Cognito, Microsoft Entra ID with certificate login, standard OpenID providers, and school-login providers like Clever.
...
### CinaAuth as an identity provider
...
* **Self-service and machine clients:** a backend registers without a logged-in user using a pre-shared token, and **Client ID Metadata Documents** (`@cinaauth/cimd`) let a client identify itself by a hosted URL, which is how MCP clients connect without being registered ahead of time. Real MCP clients like Claude and Codex now keep the authentication method they register with, confidential by default rather than forced to public.
* **Device authorization for OAuth clients:** registered CLIs, smart TVs, and other limited-input clients can ask the user to approve access in a browser, then exchange the device code for scoped, resource-bound OAuth tokens.
...
* **Conformance polish:** standard `{ error, error_description }` envelopes, `no-store` on credential responses, an `at_hash` claim on ID tokens, form-encoded requests, and an optional refresh-retry window for clients.

The old `oidcProvider` plugin, deprecated in 1.6, is removed. Move OpenID provider setups to `@cinaauth/oauth-provider`.
...
The other half of the OAuth work is about CinaAuth as the app receiving a login.
...
| **CLIs and other limited-input OAuth clients** | The device flow returned app sessions.                         | Add `oauthDeviceAuthorization()` to issue scoped, resource-bound OAuth tokens.              |
...
* **SCIM provisioning domains:** SCIM no longer depends on the Organization or SSO plugins and never represents a provisioned identity as an authentication account. Use code-defined connections, resolve application-owned connections at request time, or opt into the plugin-managed connection catalog. The new models do not convert 1.6 provisioning state, so upgrading requires a reviewed cutover and full User and Group reprovisioning.
...
Drizzle ORM v1 is now in release candidate, bringing its new relations API. CinaAuth supports it through a new adapter entry point, so the generated auth relations slot in alongside your app's own relations.

```ts title="auth.ts"
import { CinaAuth } from "cinaauth/minimal";
import { drizzleAdapter } from "@cinaauth/drizzle-adapter/relations-v2"; // [!code highlight]
import { db } from "./db";
...
`@cinaauth/i18n` ships built-in translations for 22 languages, and the English fallback is fixed. Drop it in to localize CinaAuth's user-facing messages without maintaining your own translation table.
...
| Change                                                    | What to do                                                                                                                                           |
| --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Microsoft accounts use the stable `oid` claim             | Replace each existing Microsoft account's old `sub` identifier with its verified directory `oid` before adding the account index.                    |
| SIWE derives identity from the signed message             | Remove wallet and chain fields from nonce requests; return an 8-to-250-character alphanumeric nonce from a custom `getNonce`.                        |
...
| Back-channel logout revokes session-bound tokens          | Run the schema migration, expect signed-out tokens to become inactive, and audit logout URIs for public HTTPS without credentials or fragments.      |
...
| The old `oidcProvider` plugin is removed                  | Move provider setups to `@cinaauth/oauth-provider`.                                                                                                  |
| OAuth Provider removes `silenceWarnings`                  | Delete `silenceWarnings` from `oauthProvider()` configuration.                                                                                       |
| MCP moves to `@cinaauth/mcp`                              | Update imports, endpoint paths, helper names, config shape, and schema.                                                                              |
| SAML defaults and config change                           | Update removed fields, update callback URLs, and review IdP-initiated flows.                                                                         |
| SCIM requires full reprovisioning                         | Stop provisioning, review legacy identities, replace the incompatible tables and credentials, then run a complete User and Group provisioning cycle. |
...
| Expo secure storage access is asynchronous                | Await `getCookie()`, provide asynchronous methods in custom storage, and use `storageAdapter.setItemAsync()` when writes must be awaited.            |
...
| Database joins moved out of `experimental`                | Replace `experimental.joins` with `advanced.database.joins`, then regenerate Drizzle or Prisma relations.                                            |
| Default Drizzle schema uses singular relation keys        | Regenerate and review your Drizzle schema relations.                                                                                                 |
| Device Authorization schema and OAuth grant               | Index both code columns, keep codes within 191 characters, and add `oauthClientId` and `resources` only when enabling OAuth Device Authorization.    |
...
| -------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| Refresh-token retries                  | Replay the same refresh response during a short reuse window.                                        |
...
| Drizzle Relations v2 support           | A new `@cinaauth/drizzle-adapter/relations-v2` entry point merges with your app's relations.         |
...
| Change                                  | Replacement                          |
| --------------------------------------- | ------------------------------------ |
| `oidcProvider` plugin removed           | Use `@cinaauth/oauth-provider`.      |
| MCP plugin path moved out of `cinaauth` | Use `@cinaauth/mcp`.                 |
...
Install it from the `rc` tag, and do the same for any `@cinaauth/*` packages you use:

```package-install
cinaauth@rc
```

Before applying the 1.7 schema, complete any manual preparation for account identities, OAuth clients, SCIM, and Device Authorization. Then use `npx auth@rc migrate` with the built-in Kysely adapter, or run `npx auth@rc generate` and apply the result through your Drizzle, Prisma, or custom migration workflow. The unscoped `npx auth` still resolves the 1.6 CLI until 1.7 is stable. For the required order and detailed steps, see the [1.7 upgrade guide](/docs/guides/1-7-upgrade-guide).

| If you use                                     | What to expect                                                                                            |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Basic CinaAuth setup                           | Usually the `@rc` install and reviewed schema changes                                                     |
| Social login, generic OAuth, One Tap, or SIWE  | New login-client behavior, including Microsoft `oid`, addressless SIWE nonces, and Google One Tap         |
| `@cinaauth/oauth-provider`                     | Client-data preparation plus resources, DPoP, back-channel logout, `max_age`, and safer OAuth checks      |
| MCP                                            | A package move to `@cinaauth/mcp`, official v2 clients or servers, new endpoint paths, and schema changes |
| SAML or SCIM                                   | Safer SAML and SSO checks, plus a full SCIM cutover and User and Group reprovisioning                     |
| Device Authorization                           | Indexed, bounded codes; optionally add `oauthDeviceAuthorization()` with `oauthClientId` and `resources`  |
| Magic links or email OTP                       | Safer handling for accounts whose email was never confirmed                                               |
| Expo or React Native                           | Await cookie reads and update custom SecureStore-compatible storage implementations                       |
