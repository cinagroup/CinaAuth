...
We're excited to announce CinaAuth 1.7 🎉

CinaAuth 1.7 is one of our largest releases. It brings major improvements across OAuth and OpenID Connect, enterprise identity, MCP authorization, and device access.
...
The OAuth provider is the center of CinaAuth 1.7. It already let other apps sign users in and request access. This release adds stronger client authentication, explicit rules for each protected API, DPoP, sign-out across connected apps, and broader OpenID conformance.
...
* **Explicit rules for each API:** protected APIs can now have their own permissions, token lifetime, claims, and signing policy. CinaAuth enforces which API a token was issued for (RFC 8707).
...
If you still use the deprecated `oidcProvider` plugin, migrate to [`@cinaauth/oauth-provider`](/docs/plugins/oauth-provider). The replacement provider existed before 1.7, and 1.7 removes the old plugin.
...
MCP authentication now lives in its own package, [`@cinaauth/mcp`](/docs/plugins/mcp). It uses the OAuth provider from this release, supports the MCP 2026-07-28 authorization profile, and works with version 2 of the official MCP TypeScript SDK.

The responsibilities are clear: CinaAuth handles login, consent, and access tokens. The official MCP SDK handles the messages sent between an MCP client and server.

The new package keeps the existing stateless bearer-token model and makes it safer to run across several server instances. CinaAuth still stores normal login records, such as clients, consent, and refresh tokens.
...
* **Client ID Metadata Documents:** [`@cinaauth/cimd`](/docs/plugins/cimd) lets an MCP client identify itself with a document on its own website. Older clients can still use Dynamic Client Registration when you enable it.
...
CinaAuth already supported signing into the same application from a device with limited input. A new RFC 8628 grant expands that flow to registered OAuth clients. A CLI, smart TV, game console, or IoT device shows a code, and the user approves API access in a browser.
...
This is separate from CinaAuth's existing device sign-in flow, so both can run in the same application. It also works with products that ship their own MCP command-line tool. See the [device authorization documentation](/docs/plugins/device-authorization) for both options.
...
CinaAuth 1.6 already let enterprise directories create, update, deactivate, reactivate, and remove Users. CinaAuth 1.7 rebuilds that service around isolated connections and adds first-class Groups, direct memberships, role projections, and richer employee profiles.
...
CinaAuth 1.7 replaces the legacy runtime management endpoints with 2 application-controlled options: resolve connections from your own catalog, or use the managed catalog built into the SCIM plugin.

The managed catalog supports several credentials per connection, with their own permissions and expiry dates. It can rotate and revoke credentials, record who made each change, and retire a connection safely. New secrets are returned only when they are created or rotated, while CinaAuth stores protected digests instead of plaintext values.
...
SCIM creates and manages employee records, but it does not sign employees in. CinaAuth 1.7 adds an explicit, transaction-safe bridge from a SCIM identity to a verified SSO identity.
...
* **Link the exact User:** a new SSO resolver can connect either a verified OpenID or SAML identity to an existing CinaAuth User or reject the sign-in.
...
With an exact mapping configured, SCIM and the paired login provider can point to the same CinaAuth User. See the [SSO documentation](/docs/plugins/sso) for the final setup.
...
CinaAuth as an OAuth client also received a large update. Generic OAuth providers now use the common social sign-in client path instead of a separate client plugin. PKCE is on by default, and providers configured through discovery get stronger identity-token and nonce checks.
...
### Improvements across CinaAuth

* **More built-in translations:** `@cinaauth/i18n` expands its catalog to 22 languages.
* **Drizzle Relations v2:** generated CinaAuth relations can be combined with the relations already in your application.
...
* **Safer passwordless recovery:** after a magic link or email code proves mailbox ownership, CinaAuth can remove unproven credentials and revoke older sessions.
...
* **Direct Fetch integration:** CinaAuth instances now integrate more directly, with stronger types, where a standard `fetch`-compatible handler is expected.
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Basic CinaAuth setup                           | Upgrade all CinaAuth packages together, then generate and review schema changes                        |
| Social login, custom OAuth, One Tap, or SSO    | Review how existing accounts map to identities confirmed by each provider                              |
| `@cinaauth/oauth-provider`                     | Review saved clients and apply the database changes for protected APIs, stronger tokens, and logout    |
| MCP                                            | Install `@cinaauth/mcp`, update the integration, and apply the OAuth client database changes           |
...
| Custom proxy or TLS termination                | Confirm that CinaAuth knows the public URL of your application                                         |

***

## Migrating to v1.7

Upgrade `cinaauth` and every `@cinaauth/*` package together:
