...
## CinaAuth 1.4 Release

We're excited to announce the release of CinaAuth 1.4. This release includes several new features, performance and security improvements.

To upgrade, run:

```package-install
npm install cinaauth@1.4
```
...
```ts title="auth.ts"
import { CinaAuth } from "cinaauth";
import { getOAuthState, createAuthMiddleware } from "cinaauth/api";
...
CinaAuth now supports database joins, improving over 50 endpoints by 2 to 3x in latency. This is achieved by using database joins under the hood.
...
<Callout type="info">
  Joins are supported by all adapters. When disabled (the default), CinaAuth falls back to separate queries.
</Callout>
...
The CinaAuth CLI now supports indexing your database out of the box, which greatly improves the performance of your application.
...
import { keycloak } from "cinaauth/plugins/generic-oauth";
...
you can reduce your bundle size by using `cinaauth/minimal` instead of cinaauth.
...
import { CinaAuth } from "cinaauth/minimal"; // [!code highlight]
...
[👉 Read more about Device Authorization](/docs/plugins/device-authorization)

```ts
import { CinaAuth } from "cinaauth";
import { deviceAuthorization } from "cinaauth/plugins";
...
The CinaAuth offered `reactStartCookies` plugin is now moved to `tanstackStartCookies` plugin.

```diff
- import { reactStartCookies } from "cinaauth/react-start";
...
CinaAuth uses AsyncLocalStorage for async context tracking.
...
* **ESM Only**: CinaAuth is now 100% ESM.
...
* **Error Documentation**: Common OAuth flow errors are now [documented](/docs/reference/errors).
...
👉 [Check the full changelog](https://github.com/cinagroup/cinaauth/releases/tag/v1.4.0)
