...
## CinaAuth 1.5 Release

We’re excited to announce the release of CinaAuth 1.5! 🎉

This is our biggest release yet, with over 600 commits, 70 new features, 200 bug fixes, and 7 entirely new packages. From MCP authentication to Electron desktop support, this release brings CinaAuth to new platforms and use cases.

We’re also announcing our new Infrastructure product. It lets you use a full user management and analytics dashboard, security and protection tooling, audit logs, a self-service SSO UI, and more, all with your own CinaAuth instance.

Starting with this release, the self-service SSO dashboard — which lets your enterprise customers onboard their own SAML providers without support tickets — is powered by CinaAuth Infrastructure. If you’re using the SSO plugin in production, we recommend upgrading to the Pro or Business tier to get access to the dashboard and streamline your enterprise onboarding.

And soon, you’ll be able to host your CinaAuth instance on our infrastructure as well, so you can own your auth at scale without worrying about infrastructure needs.

Sign up now: [https://dash.cinagroup.com/sign-in](https://dash.cinagroup.com/sign-in) 🚀
...
We’re introducing a new standalone CLI: `npx auth`. This replaces the previous `@cinaauth/cli` package, which will be deprecated in a future release.

```bash
npx auth init
```

With a single interactive command, `npx auth init` scaffolds a complete CinaAuth setup — configuration file, database adapter, and framework integration.
...
npx auth upgrade   # Upgrade CinaAuth to the latest version
```

The `generate` command now also supports a `--adapter` flag, letting you generate schema output tailored to your specific database adapter without needing a full CinaAuth config file:
...
The MCP plugin now ships a framework-agnostic remote auth client. If your MCP server is separate from your CinaAuth instance, you can verify tokens and protect resources without duplicating auth logic.

[👉 Read more about MCP authentication](https://www.cinagroup.com/docs/plugins/mcp)

```tsx
import { createMcpAuthClient } from "cinaauth/plugins/mcp/client";
...
```tsx
import { mcpAuthHono } from "cinaauth/plugins/mcp/client/adapters";

const middleware = mcpAuthHono(mcpAuth);
```

<Callout type="warn">
  **CinaAuth 2.x:** These 1.5 client and adapter APIs were removed. See the [MCP upgrade guide](/docs/guides/1-7-upgrade-guide#mcp) and the [current MCP plugin documentation](/docs/plugins/mcp).
</Callout>
...
The new `@cinaauth/oauth-provider` plugin turns your CinaAuth instance into a full OAuth 2.1 authorization server with OIDC compatibility. Issue access tokens, manage client registrations, and let third-party apps authenticate against your API — including MCP agents.

[👉 Read more about the OAuth Provider](https://www.cinagroup.com/docs/plugins/oauth-provider)
...
[👉 Read more about Electron integration](https://www.cinagroup.com/docs/integrations/electron)
...
[👉 Read more about i18n](https://www.cinagroup.com/docs/plugins/i18n)
...
```tsx
import { defineErrorCodes } from "@cinaauth/core";
...
CinaAuth can now resolve the base URL dynamically from incoming requests, making it work seamlessly with Vercel preview deployments, multi-domain setups, and reverse proxies.

[👉 Read more about dynamic base URL](https://www.cinagroup.com/docs/guides/dynamic-base-url)
...
CinaAuth now supports rotating `CINAAUTH_SECRET` without invalidating existing sessions, tokens, or encrypted data. When you need to rotate your secret — whether for scheduled rotation or incident response — you can introduce a new key while keeping old keys available for decryption.
...
```bash
CINAAUTH_SECRETS="2:new-secret-key,1:old-secret-key"
```
...
[👉 Read more about test utilities](https://www.cinagroup.com/docs/plugins/test-utils)
...
| Package                     | Description         |
| --------------------------- | ------------------- |
| `@cinaauth/drizzle-adapter` | Drizzle ORM adapter |
| `@cinaauth/prisma-adapter`  | Prisma adapter      |
| `@cinaauth/kysely-adapter`  | Kysely adapter      |
| `@cinaauth/mongo-adapter`   | MongoDB adapter     |
| `@cinaauth/memory-adapter`  | In-memory adapter   |

The main `cinaauth` package re-exports all adapters, so existing imports continue to work. But you can now install only the adapter you need for smaller bundles:
...
CinaAuth now natively supports [Cloudflare D1](https://developers.cloudflare.com/d1/) as a first-class database option. Pass your D1 binding directly — no custom adapter setup required.
...
The built-in D1 dialect handles query execution, batch operations, and introspection through D1's native API. Note that D1 does not support interactive transactions — CinaAuth uses D1's `batch()` API for atomicity instead.
...
* **`customSyntheticUser` option**: Support plugin fields in enumeration-protected responses ([#8097](https://github.com/cinagroup/cinaauth/pull/8097)).
...
* **`CinaAuthPluginRegistry` type system**: Typed plugin discovery via `getPlugin()` and `hasPlugin()`.
* **Version in `AuthContext`**: Access the CinaAuth version at runtime.
* **Redis secondary storage**: Extracted to `@cinaauth/redis-storage`.
...
* **Prevent email enumeration on `/change-email`**: Always returns `{ status: true }` and simulates token generation for timing safety ([#8097](https://github.com/cinagroup/cinaauth/pull/8097)).
...
The `cinaauth/adapters/test` export has been removed. Use `testAdapter` and `createTestSuite` from `@cinaauth/test-utils/adapter` instead. See the [Create a Database Adapter](/docs/guides/create-a-db-adapter#test-your-adapter) guide for details.

### API Key Plugin Moved to `@cinaauth/api-key`
...
If you are building a plugin on top of CinaAuth, there are a few things you should know.
...
| `ClientOptions`                                              | `CinaAuthClientOptions`                  |
| `LiteralUnion`, `DeepPartial` (from `cinaauth/types/helper`) | Import from `@cinaauth/core`             |
...
### `@cinaauth/core/utils` Barrel Export Removed

The `@cinaauth/core/utils` barrel export has been split into individual subpath exports to improve tree-shaking:

```diff
- import { generateId, safeJSONParse, defineErrorCodes } from "@cinaauth/core/utils";
...
```tsx
type PluginContext<Options extends CinaAuthOptions> = {
...
```diff
- import type { InferUser, InferSession } from "cinaauth/types";
- type MyUser = InferUser<typeof auth>;
+ import type { User, Session } from "cinaauth";
...
### `getMigrations` Moved to `cinaauth/db/migration` Subpath
...
```diff
- import { getMigrations } from "cinaauth";
...
👉 [Check the full changelog](https://github.com/cinagroup/cinaauth/releases/tag/v1.5.0)
