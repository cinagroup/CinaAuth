...
describe("LLM routes", () => {
	it("uses the CinaAuth product identity for latest docs", () => {
		expect(getLLMsIndexTitle()).toBe("CinaAuth Documentation");
	});
	...
	it("identifies archived documentation indexes", () => {
		...
		expect(getLLMsIndexTitle(version)).toBe("CinaAuth Documentation — v1.6");
