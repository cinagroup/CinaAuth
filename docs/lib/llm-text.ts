...
export function getLLMsIndexTitle(version?: DocsVersion): string {
	return version && version.id !== "latest"
		? `CinaAuth Documentation — ${version.label}`
		: "CinaAuth Documentation";
}
...
Use the documentation version that matches the CinaAuth version installed in the project. Find a relevant page in an index, then fetch its \`.md\` URL for clean Markdown. Cite the canonical URL without the \`.md\` suffix.

## Documentation

- [Current documentation index](https://better-auth.com/docs/llms.txt): All pages for the latest stable release.
- [Documentation MCP server](https://mcp.better-auth.com/mcp): Search and retrieve CinaAuth documentation from MCP-capable clients.
...
export function getLLMsIndex(
	...
	if (version) {
		sections.push(
			"",
			`This index covers the CinaAuth ${version.releaseLine}.x documentation.`,
		);
	}
